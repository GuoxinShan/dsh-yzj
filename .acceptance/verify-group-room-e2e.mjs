...
await ball.waitFor({ state: 'visible', timeout: 25000 })
await ball.click()
...
const groupRow = dialog.locator('button[class*="item"]').filter({ hasText: GROUP_NAME }).first()
const groupFound = await groupRow.count().then(n => n > 0).catch(() => false)
ok(`recent groups include ${GROUP_NAME}`, groupFound)
if (!groupFound) {
...
const composerInPanel = await dialog.getByLabel('输入消息').count()
ok('panel has no IM composer after picking a group (R7/H8)', composerInPanel === 0, `count=${composerInPanel}`)
ok('panel banner points at the group room', (await dialog.innerText()).includes('群房间'))
const openRoom = dialog.getByTestId('yzj-open-group-room')
if (await openRoom.count() > 0) await openRoom.click()
await page.screenshot({ path: shot('0-panel.png') })
// The floating panel overlay intercepts clicks on the room below — it must be gone.
for (let i = 0; i < 5 && await dialog.isVisible().catch(() => false); i += 1) {
  await dialog.getByRole('button', { name: '关闭' }).click().catch(() => {})
  await page.keyboard.press('Escape').catch(() => {})
  await page.waitForTimeout(400)
}
ok('floating panel is closed before room interactions', !(await dialog.isVisible().catch(() => false)))
