...
try {
  ...
  const schema = JSON.parse((await runCli('yzj-cli', ['sheet', 'get', '--id', '6a7fd194fcb864449953b34e'])).stdout)
  const table = (schema.sheets ?? []).find(t => (t.fields ?? []).some(f => f.name === 'todo_id'))
  if (table !== undefined) {
    const listJson = JSON.parse((await runCli('yzj-cli', [
      'sheet', 'record', 'list', '--id', '6a7fd194fcb864449953b34e', '--table-id', String(table.id), '--limit', '100',
    ])).stdout)
    ...
    if (ids.length > 0) {
      await runCli('yzj-cli', ['sheet', 'record', 'delete', '--id', '6a7fd194fcb864449953b34e', '--table-id', String(table.id), '--record-ids', ids.join(',')])
