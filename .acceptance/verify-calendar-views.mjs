...
ok('top tabs show 对话/日程/知识库', ['对话', '日程', '知识库'].every(label => tabText.includes(label)) && !tabText.includes('待办') && !tabText.includes('推进'), tabText.replace(/\n/g, ' '))
...
await tabs.getByRole('tab', { name: '知识库' }).click()
await page.waitForTimeout(1500)
await page.screenshot({ path: join(OUT, '6-docs.png') })
ok('docs tab still paints a domain pane', await page.getByTestId('yzj-workbench-domain').count().then(n => n > 0))
