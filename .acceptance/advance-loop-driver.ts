/**
 * Sidecar for the live six-stage loop: real yzj-cli via the bridge (no shell
 * interpolation). Playwright clicks the panel judge verbs; this file only
 * does agent-parity feeds (draft→running→decision-needed, then
 * updated→ready-for-review). Prints one JSON object on stdout.
 */
...
if (verb === 'create-to-decision') {
  ...
  const decided = await coreFeedAdvance(ctx, BUDGET, {}, caches, {
    ...
    sourceType: '对话',
    changeType: '决策请求',
    detail: '原来的理解：立项即可；现在的约束：六态走完才算闭环',
