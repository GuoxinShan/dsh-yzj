...
- **todo**：语义化待办工具族——`yzj_todo_list/create/update/complete`；稳定 ID 幂等、host 强制状态机、追加式推进日志；真机后端本地 SQLite（v1.8 决策 37，云多维表格 record 服务间歇 500 已弃用，dbt 仅剩测试 double）；tag 自由聚合现状面保留（终态复用云之家任务模型时已拍板放弃，todo-design §11.2 决策 9）；面板「任务库切换器」为多维表格时代残留，待清理；后端迁移架构见 `docs/migration/todo-backend-migration.md`
