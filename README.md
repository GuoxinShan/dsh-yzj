...
| [`packages/ui-yzj`](packages/ui-yzj/README.md) | `@dsh-yzj/ui-yzj`（`dsh.client` 双面包） | node half：`/yzj` Connection RPC 通道（含 `home-open` / `home-topic-open` / `home-send` / `home-fused` / `home-handoff`）；browser half：侧栏脚「云之家」入口块 + `conversation.view` 工作台（会话列表 \| 群房间时间线 + 话题抽屉）+ composer takeover「发进群」+ 话题锚点卡 / 未绑定「丢进群」+ 悬浮球工作台（P0 仍在；待办/日程/知识库暂开旧面板）+ **设置 → 云之家** |
...
- **云之家工作台**：悬浮球唯一入口，四个 tab——知识库、日程、会话（挑群打开 DSH 群房间，不再在面板发消息）、**待办**。群房间与话题见 [group-room-topics.md](docs/spec/group-room-topics.md)，对照 gap §23。
...
- **会话家园 v2.0 第一刀已落地**：群房间 + 话题（`yzj-home-*` / `yzj-topic-*`）；官方 Chat tab 仍并存（不能替换 harness tab ring）。仍开放：确认卡 pending 不进 session 日志、无群搜索、既有宿主 ③④ 历史迁移（[gap-analysis §23](docs/status/gap-analysis.md) H2/H9，§22 G3/G5）。
