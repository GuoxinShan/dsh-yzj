...
> v1.2 增补：实现进展状态全量刷新（git 至 `491db61`）——P0 消息回源、P1 门禁分级 + 确认卡、通知层一/三、skill、影子任务库、dbt 预览均已落地；剩余缺口见 §15 更新清单。v1.1 增补：补全实现侧全部工具明细（§2）；新增设计补强「悬浮窗全量拖拽 → composer」（§2A，用户思路）。

**当前完成度：约 85%**（按设计 v1.6 验收基准）。剩余：第二层通知的「通知卡按钮」受限降级、@ 候选三组排序、锚点定位高亮、拖入即处理引导、多 chip 合并、快照决策标注（待拍板）。
...
| 4. 拖入即处理引导（P2 可选） | ❌ 未实现（drop 后无快捷动作；不阻塞既有管线） | ⚪ P2 |
...
| 设计点 | 实现 | 状态 |
|---|---|---|
| InputTriggerSource | ✅ 单组「云之家」（知识库 + 会话 + 查询时联系人） | 🟡 |
| 三组候选（同事 order=0 / 群 order=1 / 文档 order=2） | ❌ 未分组（文档候选缺） | 🟡 P2 |
| @同事 越权边界 UI 明示 | ❌ | 🟡 P2 |
| @同事 起草消息给他 | ❌ | 🟡 P2 |
...
## 9. 面板跳转服务 yzjPanel（§5.2）— 🟡 主体达成

确认卡「查看上下文」已实现（打开面板 + 切 tab + 加载群消息/workspace 文档）；缺「滚动定位锚点消息并高亮」（P2）。
...
### 🟠 P1（接近完成）
1. **锚点定位高亮**：确认卡/通知场景「查看上下文」滚动定位锚点消息并高亮（面板消息列表 + 锚点参数）。

### 🟡 P2（体验细化，均可选）
2. @ 候选三组（同事/群/文档）分组与排序 + 文档候选 + 「起草消息给他」。
3. 拖入即处理引导（drop 后快捷动作）。
4. 多 chip 合并序列化、chip 灰化（源失效）。
5. @同事 越权边界 UI 明示。
6. 通知层二增强：播报文本内嵌群锚点（受事件限制，以文本形式给群 id 即可）。
...
### 机制边界（文档备案，不实现）
- `yzj.write/request|resolved` 持久化事件族（harness 白名单限制，由 host 内存 + 官方事件承载）。
- 通知卡（ConversationNodeDefinition 渲染的按钮卡）同因受限。
