...
> 一句话：**当前多维表格只是 demo 载体**。全部业务不变量（稳定 ID、状态机、幂等、追加日志、#tag 聚合）都收在 host 侧的 todo 核心层，后端只当存储；切换原生待办后端时**换适配器不换工具面**。
...
──────────────────────────────────────────────────────────────
存储适配层（可换）  SheetTodoStore ← 当前实现：一张多维表格「待办任务库」
                    （自动发现/开通；字段映射见 §2；CLI 格式适配见 §3）
```
