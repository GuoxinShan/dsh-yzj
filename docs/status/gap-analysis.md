...
| H11 | 导航 | **v1.12 / R28**：侧栏脚只有一个「云之家」入口（`yzj-dock-home`），点它 focus 房间、不切域。四域是工作台顶栏页签（`yzj-workbench-tab-*` / `setWorkbenchDomain`）。对话 = 会话列表 + 时间线 + 话题抽屉；待办/日程/知识库 embed 原面板。**记忆入口搁置**（R21 v1.6）：dock / 工作台 / 设置分段不露，包与工具保留 | 左边一个入口；页签切域；单聊无抽屉；群聊 header「话题 N」开关抽屉 | ✅ P2 关闭；记忆面 v1.6 卸入口；v1.12 收成单入口（dock / room-shell 单测） |
| H12 | 模型上下文 | **v1.9**：`formatSummonWindow` 头块仍是契约。真机曾空窗（pitfall-027）：T5 只查房间表、`none` 被 skip、注册不在 agent assemble 层。现：`summonWindowText` 兼查话题、skip 只跳 plugin、inject-scope mixin；抽屉问助手先 inject 再 followup | 话题里问助手能对群发/回复；snapshot 有 `yzj-bound-window` | ✅ 关闭（digest 单测 + summonWindowText + ask inject；真机 `.acceptance/verify-summon-window.mjs`：inject + snapshot 均含 `本群最近消息` / `groupId` / 锚点，模型复述正确） |
...
| H24 | 工作台首帧 | 侧栏脚「对话」曾 `await homeNav` 再 `homeOpen` 才 focus，冷 CLI / 重开座位时左下角要点很久。现：已在房间再点「云之家」不换域、不重开座位；否则 focus 上次座位（模块缓存 + dock 预取），`home-open` 后台补 | 点「云之家」立刻出工作台 | ✅ 关闭（dock / home-focus 单测） |
...
| H32 | 话题结果不回云之家话题 | DSH 话题里问助手，回复只留在 DSH；群里锚点回复链是空的。模型偶尔调 `yzj_im_message_send`（还要确认卡），且会把过程也发出去。v1.10 / R26：`agent/status` idle 后 host 以 CLI 本人身份把本轮**最后一条助手正文**回复到 `rootMsgId`；本轮 `write`/`edit` 产物 `file upload` 后随附（图进 richText 回复；其它文件因 CLI 不支持 file+reply，跟发群时间线，总结写明落点，pitfall-028）。跳过 plugin 入站 / 已调 `yzj_im_message_send` / 写闸 pending / 假锚点 / 无正文。机器人通道本刀不动 | 云之家话题链能看见总结 + 产物；不是每条气泡 | ✅ 关闭（`topic-deliver` 单测 + dsh-2 真机回帖；file 不能进链是 CLI 契约不是漏实现） |
| H33 | 话题面板看不到产物 | 云之家 file 不能挂回复链，话题抽屉透镜原先只有文字气泡。v1.11 / R27：透镜把本轮 write/edit 文件画在助手气泡下。R26 发群照旧 | 抽屉里能看见纪要.md 等卡片；群里文件帖仍在 | ✅ 关闭（`topicLensBubbles` 单测 + 抽屉客户端卡） |
| H34 | 四域多个侧栏入口 | 侧栏脚并排「对话 / 待办 / 日程 / 知识库」四钮，和官方侧栏抢注意力，切域还跟打开工作台缠在一起。v1.12 / R28：左边只留「云之家」；四域改工作台顶栏页签。卡片「查看」仍 `setWorkbenchDomain` | 一个入口 + 页签 | ✅ 关闭（dock 无四钮；room-shell 顶栏页签单测） |

沿用不动：消息日志存储/去重/回填（T1/T7–T9）、召唤窗口（T4/T5）、写路径 D9、群内建议卡（ConfirmBroker）、未绑定私聊与丢进群（D7/D8）。G3 与 G5 继续开放。**v1.10 / R26**：话题 job-done 投递是「问助手」的契约投递面（CLI 本人身份、无确认卡），不是 D9 agent 另起写；agent 显式 `yzj_im_message_send` 仍走确认卡。

### 23.2 验收指针

按 [`group-room-topics.md`](../spec/group-room-topics.md) §7 + §9.7。H2/H3/H5/H6/H7/H8/H9/H10/H11/H12/H13/H14/H15/H16/H17/H18/H19 有单测。H4 入站话题有 router 单测，`local-*`→真实 msgId 的 `retargetAnchor` 有 topics 单测；真机 `.acceptance/verify-robot-at-topic.mjs`（未连接 skip）。真机脚本：`.acceptance/verify-group-room-e2e.mjs`（需运行中 GUI + 已登录 yzj-cli；**禁止杀 3080 / `--profile web` 宿主**——改 host / browser 后请用户手动重启 GUI；改 browser TS 后 bundle 前必须先 `tsc -b`，见 pitfall-016）。**v1.1 P0**：入口块 + 会话列表 + 话题抽屉 + `lastActivity` + `~/.dsh-yzj/workspace`。**P1**：时间线精致度六条。**P2**：四域迁入工作台、`shell.overlay` 摘除、72px 留白删除。**P3**：`TopicRecord.status` + L2 徽标 + write-gate L5 回落。**视觉刀**：pitfall-018 tab ring、composer 列、dock 退役、群名占位。**宿主生命周期**：pitfall-019 总线 + view-ring 收窄到 header + e2e 盒子缺失即失败。**v1.2**：H9 历史对话话题、H18 透镜气泡/问助手、H4 skip 型 e2e、发布口径。**v1.3 布局刀**：`.acceptance/verify-room-layout.mjs`（有界三栏 / 触底 / composer 可见 / 无鬼影行 / 机器人标注 / 长帖折叠 / 抽屉有界）；pitfall-020（overlay 契约 + 跟随门控）；H10 标题改「话题 · 群名」。**v1.4**：R20/H16/H21 云之家分组只进话题；`attachYzjSession` 闸 + cwd/RPC 单测。**v1.5**：R22/H23 话题与普通会话不得挂 IM 壳；view-ring 点「对话」+ `YzjRoomShell` 前缀闸。**v1.6**：R21/H11 记忆入口搁置；R23/H26 拖入引用退役（面板/待办无 drag，`verify-drop.mjs` skip）。**H27**：他人气泡浅色主题可见（pitfall-023）。**v1.7**：R24/H28 点群只切 groupId，不建/不 focus DSH 会话（pitfall-024）。**R25/H29**：话题不写空 turn 1（pitfall-025）。**v1.9 / H12**：话题问助手近窗（pitfall-027；`.acceptance/verify-summon-window.mjs`）。**v1.10 / H32**：话题 job-done 投递（`topic-deliver.spec.ts`；dsh-2 真机回帖自跳过若未登录）。**v1.11 / H33**：话题透镜产物卡（`topicLensBubbles` + 抽屉客户端；发群 R26 仍在）。**v1.12 / H34**：侧栏单入口 + 工作台页签（`yzj-dock-home` / `yzj-workbench-tab-*`；dock / room-shell 单测）。
