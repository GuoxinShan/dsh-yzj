...
| Occupancy | 不占 layout `conversation` / `sidebar.workspaces` 单座；占 `conversation.view` + composer chain 画 null；**CSS+DOM 收起官方 InputBar/统计/session chrome**（不依赖 `data-composer-seat`，pitfall-052）；CSS 藏 New Session / 文件夹树 / details / tablist | pitfall-050 / 052；`host-chrome.ts`；`host-chrome.client.spec.tsx` |
