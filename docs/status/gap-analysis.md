...
| H32 | 话题结果不回云之家话题 | DSH 话题里问助手，回复只留在 DSH；群里锚点回复链是空的。模型偶尔调 `yzj_im_message_send`（还要确认卡），且会把过程也发出去。v1.10 / R26：`agent/status` idle 后 host 以 CLI 本人身份把本轮**最后一条助手正文**回复到 `rootMsgId`；本轮 `write`/`edit` 产物 `file upload` 后随附（图进 richText 回复；其它文件因 CLI 不支持 file+reply，跟发群时间线）。跳过 plugin 入站 / 已调 `yzj_im_message_send` / 写闸 pending / 假锚点 / 无正文。机器人通道本刀不动 | 云之家话题链能看见总结 + 产物；不是每条气泡 | ✅ 关闭（`topic-deliver` 单测 + dsh-2 真机回帖） |
