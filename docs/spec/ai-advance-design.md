...
> 版本：v1.9（决策 43：待决决策单卡综合+出口人主权；决策 44/45：领域模型收敛——行动=事元载荷、闭环强制执行→再观察，见 [`advance-domain-model.md`](advance-domain-model.md)；决策 42：移除 bundle 的 `schedule`/`time-context` 挂载行——决策 35 已把巡检收敛为 host 机械 routine，模型面 `schedule_create` 自此无消费者；v1.7 = §17 Dream 蓄水池落地，决策 33/34；v1.6 = ④期知识沉淀出口 §16 + 决策 26–31 含第七态 `cancelled`；v1.5 = 订阅模型一等化（当时名「意图线程」，v1.8 改名上下文来源；v1.7 决策 32 加 dir: 目录级订阅）；v1.4 = 主动发现 scan → inspect → feed 巡检回路，见 §14；v1.3 = 打扰判据 + 确认卡门控线收窄，见 §13）
