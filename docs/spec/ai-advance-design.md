...
| 45 | 闭环强制：执行→再观察（v1.9，**待实现**） | **执行事元必须带效应指针 refs**（todoId / `im:<groupId>:<msgId>` / eventId / sessionId）+ 执行后效应对象自动 `sourceAdd` 进事项订阅集——host 强制，缺失拒写；面板 done 态从事元流折叠重建（废止客户端 `doneActions` 内存 Set） | 推进闭环（观察→分析→决策→执行→再观察）的「执行→再观察」弧当前是断的：决策卡建待办无 refs 无订阅（回流断）、发消息无 msgId 指针（溯源断）、done 态刷新即丢。与决策 41「decision-needed 必须配决策请求」同型——闭环纪律从文档进代码 |
