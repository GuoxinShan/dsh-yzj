...
> 版本：v1.10（**决策 49：推荐订阅源**——refs 反推 + Dream 跨事项两路推荐，三道降噪闸，货架不是账单，见 §15.6；v1.9 系列：决策 43–48 单卡综合/领域模型/闭环强制/分流判据/演进折叠/todo 渠道）；v1.7 = §17 Dream 蓄水池落地，决策 33/34；v1.6 = ④期知识沉淀出口 §16 + 决策 26–31 含第七态 `cancelled`；v1.5 = 订阅模型一等化（当时名「意图线程」，v1.8 改名上下文来源；v1.7 决策 32 加 dir: 目录级订阅）；v1.4 = 主动发现 scan → inspect → feed 巡检回路，见 §14；v1.3 = 打扰判据 + 确认卡门控线收窄，见 §13）
...
**PRD**：《AI推进-产品PRD v2.1》（内部摘录，不保留来源指针）。关键锚点：
...
**原型**：内部 HTML 原型的 lgap17 版「AI 推进」页。函数锚点：`queueHtml`（三栏目队列）/ `detailHtml`（kicker+meta+指标卡+目标+决策+时间线的主详情）/ `metricsHtml`（成功指标卡行）/ `timelineHtml`（三色时间线+空态 hero）/ `decisionHtml`（阶段化决策区）/ `sideHtml`（信息来源+已有产物右栏）/ `startModal`（发起推进弹窗）。UI 复刻口径见 §7。
...
在**同一「待办任务库」dbt** 里新增两张表（`sheet table create` 对存量 dbt 可用；新表 SingleSelect 可预注册，绕开运行期加选项被静默丢弃的坑）。库切换器/团队库语义天然复用：解析先走 todo 的 `resolveLibrary`（面板激活库 override → 显式配置 → 发现/开通），再在该 doc 内找/开通推进双表；用户切换任务库时推进事项跟随同一 doc。
...
读路径：事项列表 = 「事项」表全量 + host 排序；详情 = 按 `advance_id` filter「事元」表（filter Equals 已实测可用）。写路径：每次 feed = 追加一行事元 + 回写事项投影字段（last-write-wins）。**两次写非事务**：投影写失败时事元行已在——读路径以事元流为准（`最新动态` 只是缓存）；原生后端应服务端折叠。
...
| 20 | 上下文来源订阅存哪（v1.5；v1.8 改名） | **host storage-domain `yzj_advance_sources`**（v1.8 前名 `yzj_advance_threads`，open 时 legacy 迁移；advanceId → 来源 tokens），不动 dbt 双表 schema | 存量事项表加列要动已 provision 的表；storage-domain 与 cursor（决策 18）同模式即可落地。**这是 demo 落位不是合同** |
...
| 23 | 策略选择的载荷（v1.5） | 决策请求事元的 `变化内容` 按行约定备选（`选项N: 描述`，末行可 `影响: …` 复述）；决策区渲染为可选项，用户选定经 judge `confirm_advance` 带 note 落 user 事元。**MVP 文本约定** | PRD §5.2.3 最小推进回路要求「AI建议+备选+自定义」；0819 会议演示（私有化 → 加资源/延期/自定义）确认这是显式一步；demo 存储无结构化字段可用 |
| 24 | MVP 与灵基终态的关系（0819 会议） | **合同 + 证据 → 重建**，不是原型移植。终态文档已迁出本仓，不再维护 | 会上拍板终态要灵基侧重新组装；本仓只维护 MVP |
| 25 | 「同源」去重的判定口径（真机实验后收窄，修订决策 19） | **refs 集合完全相等 + 同一 changeType 才算重放**（幂等返回、不追加）；refs 部分重叠正常追加，返回 `overlappedRefs` 提示 | 真机实验（gap §24.6）实测：交集口径误吞「目标更新」——不同事元合法引用同一文档（回放② refs=[0806,0812 纪要]、③ refs=[0812 纪要] 被幂等吞掉，模型追问中断、补救多弹一张卡）。交集口径把「引用同一证据」误等同「同一信号重放」；真正的重放是 refs 集合与语义类型都相同 |
...
| 30 | 复盘文档默认落点 | **「我的知识/推进复盘/<事项名>」自动建父目录**（可选手改） | 「实验目录」父目录模式已验证（演示/清理两便）；一个事项可多次复盘，目录归档系列 |
| 31 | ④期实现范围 | **教学面 + 模板 + 终局提示 + cancelled 态**；工具面零新增（get/import/feed 全复用）；④-b 纪要出口只做模板+流程打包，群里转录自动感知留⑤期 | 第 0 波证明现有工具链人工已跑通（10 分钟/4 篇基线）；④期价值是流程打包，不是新能力 |
...
| 33 | Dream 的采集模型 | **蓄水池**（DreamPool）：Work scan 的新信号 copy 入池 pending，Dream 触发时统一提炼；替代决策 21 的「Dream 每日直取订阅流」合同（直取无法攒批折叠） | 用户拍板 eventloop 设想：「定时把待抽取进 eventloop，到一定数量或时间就开始抽取事元产生建议卡片」；Work 即时价值（已验）不动，池是待抽取队列不是替代 |
...
| 43 | 待决决策的存续语义（v1.9） | **单卡综合 + judge 结算 + 待决出口人主权**：① 卡面永远只有一条当前决策（实效性）——已有未处理决策请求时，新判据命中不并列起卡，agent 必须 feed 一张**综合卡**（detail 带「综合自： <旧卡 entryId>」写明旧问题的并入/失效；host 强制校验，缺标记/缺 id 直接拒绝）；② judge 事元带 `判定动作` 标记，卡面=最近判定之后的最新决策请求，处理完重算；③ host 强制：decision-needed / ready-for-review 的离开只由用户（judge actor=user）执行，agent 试图拖出直接拒绝——与决策 27 终局主权同构 | 用户拍板两轮：「产生的决策会给下一次 dream 覆盖掉？这不太对」→ 先上队列；「应该只有一条决策，但后续要根据这个综合起来产生新的才对，因为会有实效性」→ 队列修正为单卡综合：内容不丢（综合自链）+ 卡面不胀（只有一条）+ 阶段不被拖（出口闸） |
...
| 44 | 行动的建模形态（v1.9，领域模型收敛） | **不设独立行动实体/表**：行动 = 决策请求事元的结构化载荷（动作行，决策 41）+ 执行事元 refs 留痕；预留实体化接缝——当 agent 执行器（派任务）出现且「行动清单」升为一等视图时，同一结构 `{kind, text, fields, effectRef, proposedBy}` 平移为独立 action 面，事元留痕不变。推翻信号：「派出去的事跑到哪了」答不上来 / 同一动作因 done 态丢失被执行两次 | 08-21 领域建模讨论收敛：行动执行器多态（人 todo / IM / 日程 / 未来 AI），执行态归各执行器域，独立建表 = 双事实源；事元流只载已发生事实，行动是未发生意图，混入破坏 append-only 合同。全貌见 [`advance-domain-model.md`](advance-domain-model.md) §3 |
| 45 | 闭环强制：执行→再观察（v1.9，**已落地**） | **新 RPC `/yzj advance-action-run`：host 编排三效应原子**——执行（todo/im/event）→ 执行事元留痕（refs=效应指针 todoId / `im:<groupId>:<msgId>`，detail 带 `动作序: <key> \| 种类 \| 文本` 标记）→ 效应对象自动 `sourceAdd` 订阅；幂等闸（同动作序或同 kind+文本已留痕则不双执行，防综合卡重排后复点）；效应失败整体不落事元，订阅失败降级 warning。面板 done 态废止内存 Set，改从事元流折叠（`foldDoneActions`，刷新不丢） | 推进闭环（观察→分析→决策→执行→再观察）的「执行→再观察」弧原先是断的：决策卡建待办无 refs 无订阅（回流断）、发消息无 msgId 指针（溯源断）、done 态刷新即丢。与决策 41「decision-needed 必须配决策请求」同型——闭环纪律从文档进代码 |
| 46 | 推进事项 vs 待办的分流判据（v1.9） | **完成标准自明、一句话说清的单动作 → `yzj_todo_create`；有业务目标 + 成功指标 + 需跨时间跟进判断 → `yzj_advance_create`；拿不准建待办**（轻、可逆，做一半发现需持续跟再升级立项——待办完成经 `todo:<id>` 订阅回流为事元）。两个工具 description 互引落实 | 用户拍板「听你的」。此前两工具 description 互不提对方、纪律面真空，agent 建哪个全靠自悟；判据本质：不确定性在「做不做」=待办（执行问题），在「做到什么程度」=推进事项（判断问题）。立项是重承诺（看板占位+巡检比对+验收回路），宁轻勿重 |
| 47 | 演进折叠 = 延迟决策（v1.9） | **事元流之上永不做实体摘要层**（双事实源破「流是事实」）；视图层折叠（渲染时按段折叠、不落库）也不预支——触发信号 = 用户读时间线说「太长了看不懂这件事经历了什么」；届时折叠键选**目标版本段**（每次「目标更新」事元之间的区间），不选日历周（目标改变才是演化的节拍） | 08-21 讨论收敛：用户质疑「几个事元不再压缩一层怎么表现影响」——区分出影响的三个既有落点（写时判定的 changeType/detail 推论链/阶段流转 + 读时投影的指标卡）与缺的「影响叙事」；叙事折叠现在无长度压力证据，预支会重蹈「进度节点分组」弯路（同日拍板删过的层） |
...
`YzjAdvanceService.startPatrolTimer()`：setInterval ≥300s，tick = `patrolNow()`（coreScanAdvance 全量聚合，错误吞掉——巡检永不弄坏 host）。`ctx.effect` 注册，bundle 卸载清 timer。面板「巡检」按钮 = `/yzj advance-patrol-now` 立即机械一轮（无模型、不切域、不写 ask）。旧「巡检五步」模型教学面作废——真机实验观察到模型实时判断漂移（同一信号集一次拒噪音一次聚合），判断权收敛到 Dream 单点。
...
### 15.6 推荐订阅源（决策 49，v1.10）

> 出处：用户拍板「自动推荐订阅源降低摩擦，不要让用户手动订阅确认就好」+「不能一下推荐几百条」的降噪要求。

**目标**：订阅摩擦归零——用户不手动找渠道；系统推荐、用户只点确认。

**扫描范围（零扩张，硬约束）**：不推荐没跟事项发生过关系的渠道；不做隐式全群扫描（决策 17 边界不动）。推荐素材只有两个证据来源：

- **① refs 反推（host 机械路，feed 时实时）**：新事元落库后，host 从 refs 提取渠道 token（`im:<g>:<m>` → 群 `im:<g>`；`doc:` → 所在库整库）——不在订阅集 ∪ 已忽略集 → 落推荐事元。零模型成本，证据驱动（这渠道已被引用过）。
- **② Dream 跨事项（模型路，随抽取节奏）**：池信号与事项强相关但该事项未订阅其渠道 → 产出推荐。dreamAskPrompt 加一条纪律。

**推荐事元**：`changeType=备注`、`actor=agent`，detail 带标记行 `推荐订阅: <token> | 理由`。推荐事元自身豁免推荐检查（防递归）。

**降噪三闸**：① 同一渠道终身只推一次（已有未结算推荐 → 幂等不重复）；② 不回溯历史（只在新事元落库时检查，上线零爆发）；③ 忽略永久（× 落忽略事元，同 token 永不推，host 强制）。

**呈现（货架不是账单）**：右栏「上下文来源」区底部灰字行「＋N 个推荐来源」，展开见列表，每条 `[挂上] [×]`。**不进「待我决定」、不进队列、不占主视觉**——可选项，不点无任何后果（与决策卡的「必须拍板」本质不同）。

**结算**（与决策 43 单卡结算同构）：挂上 = 既有 `sourceAdd` 直写 + 确认事元（`推荐确认: <token>`）；忽略 = 忽略事元（`推荐忽略: <token>`）。折叠规则：推荐事元存在且其后无同 token 确认/忽略事元 → chip 显示。

**写门禁**：推荐事元纯追加静默落；挂上/忽略 = 用户直写无卡（D9）。**订阅边界始终人划**——推荐只点亮，不自动挂。

**实现面**：`coreFeedAdvance` 尾挂推荐检查（~70 行）+ 面板推荐行折叠与按钮（~90 行）+ Dream prompt 纪律一句 + 两侧测试。

## 16. 知识沉淀出口（④期，决策 26–31 已拍板）

> v1.6。依据：硬要求 ②（完整时间线=知识沉淀来源）+ 真机实验第 0 波人工基线（「转录→四段式纪要→入库」agent 口径约 10 分钟/4 篇，gap §24.6）+ 事项 A-20260819-002 的 goal 本身（「跑通 听会→标准纪要→共识入知识库→下一步入任务 的最小回路」——纪要出口是这个产品对自身的兑现）。**复盘 = 终局收口**（决策 26）：做完了（completed）或黄了（cancelled，决策 27）才沉淀；中途回看靠用户口述，不设专门入口。
...
| **④-b 会议纪要** | 一份会议转录（群文件 fileId / 本地文件 / 知识库 docId） | 金蝶标准四段式纪要（目标/内容/共识/下一步）入知识库；「下一步」逐条挂入相关事项（feed 事元，refs=[纪要 docId]） | 用户把转录交给 agent（对话/喂给推进），后续可接 测试群话题自动感知（⑤期依赖） |
...
- 读：`yzj_advance_get` 翻页读全量事元流（既有，真机实验已验证 5 条全量读出）。
...
零新工具的理由：第 0 波证明现有工具链已能人工跑通；④期的价值是**流程打包与触发面**，不是新能力。若拍板要「一键沉淀」（面板直写不经模型），才需要 `yzj_advance_export`（host 折叠模板，无 LLM）——草案不推荐：复盘品质恰恰靠模型读流后的归纳，host 模板折叠只是拼接。
...
3. 测试事项（A-20260819-002）实测复盘：口述「复盘一下 测试事项」→ 模型读全量事元 → 复盘 md（16.2 五段）→ `doc import` 弹卡 → 确认入库 → 回链事元出现在时间旅程与「已有产物」；全程确认卡恰好 1 张。
...
1. scan 发现信号 → 池 pending 增加;Work 即时 feed 行为不变(即时路径回归)。
...
5. `pnpm test` 绿;真机演示路径(关联 测试群 → scan 入池 → 手动抽取)全通。
