...
「分析」（比对/判据/Dream 抽取）是 LLM 判断服务，**过程不是实体、不留痕**（830 实验实测模型判断漂移，过程不可靠）；但判断的**非空产出强制实体化为事元**——结论 + 推论链（detail）+ 证据（refs）三件套：
...
## 5. 断点修复记录（2026-08-21 决策 45 已落地）

原三断点（决策卡动作的执行→再观察弧开口）已由 `/yzj advance-action-run` 闭环：

1. ~~回流断~~：执行事元必带效应指针 refs + 效应对象自动 `sourceAdd` 订阅（host 编排，幂等闸防双执行）。
