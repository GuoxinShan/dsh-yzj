...
> 版本：v1.4（S11 双后端拆除；S9 切换器退役 / S10 归档；期①②已落地）
> 日期：2026-08-25（v1.4/v1.3）；2026-08-24（v1.2/v1.1/v1.0）
> 决策人：Guoxin Shan
...
待办页签从列表改五列泳道：**待我决定** | 可认领 | 进行中 | **待我验收** | 已完成（已终止、**已归档**各收进折叠区，与推进看板「已结束」同款）。列名与推进三栏目同词（决策 52）——两个看板各只有两列会来找人。卡片操作即状态动词（批准/打回/验收）；卡片支持行内编辑任务详情（S7）；拖动留到后续。**任务库切换器已退役（S9）**——真机后端本地 SQLite 后它是纯残留，面板不再渲染。
...
| S9 | 任务库切换器（v1.3，2026-08-25） | **面板撤除切换器整行**（个人/团队库切换、团队库开通、localStorage 偏好）；dbt 发现代码路径保留给测试（backend='dbt' 分支不变） | 真机后端本地 SQLite（决策 36/37）后切换器是纯残留——偏好不被读取、扫描白白花十几次 CLI 调用；用户拍板「不要了」。恢复信号：未来回到云后端/多库 |
| S11 | 多维表格双后端（v1.4，2026-08-25，决策 54） | **拆除——本地 SQLite 是唯一后端**：`setTodoBackend` 开关、dbt 发现/开通/切换（bindingForDoc/provisionTable/listLibraries/select/ensureTeam）、`todo-libraries/select/ensure-team` RPC 全删；`yzj_sheet_*` 工具族保留（agent 的通用能力，与 todo/advance 存储无关） | 云 dbt record 服务间歇 500 早已弃用（决策 36/37 切 SQLite），双分支是纯死代码；用户拍板「干掉」。测试从 fake-bridge 脚本改为 sqlite 直验（tests/sqlite-harness.ts） |
| S10 | 归档（v1.3，2026-08-25） | **归档 = 视图层隐藏标记（非第七态，与状态机正交）**：done/cancelled 卡片「归档」→ 收进「已归档」折叠区 → 「恢复」回原列；`yzj_todo_list`（含 all）排除已归档；标签轨只统计在途卡；不增版本不动状态，日志留痕 | 已完成/已终止会无限堆积占满板面；用户拍板「能不能归档」。不做定时自动归档（不预支）也不做删除（历史事实保留） |
