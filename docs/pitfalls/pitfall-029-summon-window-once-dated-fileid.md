...
话题会话（例 `yzj-topic-gid-test-…`）里每敲一轮，runtime snapshot 都带完整 `［本群最近消息］`（默认 20 条 / 4000 字）。同轮第二个快照又把它丢掉（2741↔577），记忆库明明没变也跟着整段重发。窗口第一行 `[16:55]` 下一行 `[10:50]`，看起来像倒序。群文件只有 `[文件]:名`，模型拿 msgId 调 `yzj_file_download` 得到 API 2001。
