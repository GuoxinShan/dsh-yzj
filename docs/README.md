...
docs/
  ...
  spec/                              ← 设计基线（要做什么、为什么、验收口径）
    ...
    ai-advance-design.md               AI推进（**v1.10**）：①期双表/七态/第五页签；②期用户直写 feed +「现在反馈」；③期 inspect +「请 AI 验收」；§13 门控线；§14 主动发现 scan→inspect→feed；§15 上下文来源订阅（含 dir: 目录级 + §15.6 推荐订阅源，决策 32/49）；§16 知识沉淀出口（cancelled 第七态 + 复盘/纪要模板）；§17 Dream 蓄水池（决策 33/34）；决策 1–50
