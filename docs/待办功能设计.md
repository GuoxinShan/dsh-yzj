...
> 版本：v1.1（已拍板并开工，2026-08-15）
> 日期：2026-08-15
> v1.1 变更：**tag 升级为核心理念**（§0）；全部待拍板项已决策（§12）；实测格式修正 schema（负责人 Contact→MultiLineText 降级、标签 MultipleSelect→`#tag` tokens 降级，探针证据见 §11）；实施进度见 §13；迁移架构独立成文 `docs/待办后端迁移说明.md`。
...
| # | 决策 | 结论 |
|---|---|---|
| 1 | 库落位与可见性 | **双轨**：缺省个人库自动发现/开通（个人知识库 → 「待办任务库」）；`todo` 配置（workspace/docId/tableId）支持显式指向团队库。分派同事场景由团队库承载，`scope` 工具参数暂不引入（配置级区分够用） |
