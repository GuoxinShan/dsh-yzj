window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SheetBody(meta) {
			const sheets = asArray$13(asRecord$21(meta.schema).sheets);
			if (sheets.length > 0) return (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: sheets.map((item, index) => {
					const table = asRecord$21(item);
					const fields = asArray$13(table.fields).map((field) => asString$14(asRecord$21(field).name)).filter((name) => name !== "");
		...
		function ImBody(meta, openPanel) {
			...
			if (messages.length > 0 && asString$14(asRecord$21(messages[0]).sendTime) !== "") return (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: messages.map((item, index) => {
					...
					const reply = asString$14(asRecord$21(message.param).replySummary);
			...
			if (groups.length > 0) return (0, react_jsx_runtime.jsx)("div", {
				...
				children: groups.map((item, index) => {
					...
					const last = asString$14(asRecord$21(group.lastMsg).content);
		...
		function TodoBody(meta, toolName) {
			...
			const link = asString$14(asRecord$21(meta.library).link);
		...
		function YzjHomeChrome(props) {
			...
			(0, react.useEffect)(() => {
				...
				const tick = async () => {
					...
					const topic = asRecord$20(raw.topic);
		...
		function HandoffModal(props) {
			...
			(0, react.useEffect)(() => {
				props.fetchGroups(20).then((result) => {
					if (!result.ok) return;
					const rows = asArray$12(asRecord$20(result.value).list).map((item) => {
				...
				props.homeDigest(props.sessionId).then((result) => {
					if (!result.ok) return;
					const list = asArray$12(asRecord$20(result.value).candidates);
					setCandidates(list);
		...
		function YzjSessionShell(props) {
			...
			(0, react.useEffect)(() => {
				...
				const tick = async () => {
					const result = await props.homeBinding(props.sessionId);
					if (cancelled || !result.ok) return;
					const binding = asRecord$19(asRecord$19(result.value).binding);
					setRoomKind(binding.yzjKind === "dm" ? "dm" : binding.yzjKind === "group" ? "group" : "");
				};
				tick();
				const timer = window.setInterval(() => {
			...
			const handle = (0, _deepseek_ai_dsh_client_runtime_client.defineStore)({
				...
				actions: {
					...
					appendGroups: (d, groups) => {
						const seen = new Set(d.groups.map((group) => String(asRecord$18(group).groupId)));
						d.groups = [...d.groups, ...groups.filter((group) => !seen.has(String(asRecord$18(group).groupId)))];
					},
					setGroupId: (d, id) => {
						d.groupId = id;
					},
					...
					prependMessages: (d, messages) => {
						const seen = new Set(d.messages.map((message) => String(asRecord$18(message).msgId)));
						d.messages = [...messages.filter((message) => !seen.has(String(asRecord$18(message).msgId))), ...d.messages];
					},
					appendMessages: (d, messages) => {
						const seen = new Set(d.messages.map((message) => String(asRecord$18(message).msgId)));
						d.messages = [...d.messages, ...messages.filter((message) => !seen.has(String(asRecord$18(message).msgId)))];
					},
					setAnchorMsgId: (d, id) => {
						d.anchorMsgId = id;
					},
					...
					patchTodo: (d, todo) => {
						const todoId = String(asRecord$18(todo).todoId);
						d.todos = d.todos.map((item) => String(asRecord$18(item).todoId) === todoId ? todo : item);
					},
					setTodoTag: (d, tag) => {
						d.todoTag = tag;
					},
		...
		*/
		...
		function rowsOf(json) {
			...
			const record = asRecord$17(json);
			if (Array.isArray(record.list)) return record.list;
			if (Array.isArray(record.data)) return record.data;
			if (typeof record.data === "object" && record.data !== null) {
				const inner = asRecord$17(record.data);
		...
		function parseContactUser(json) {
			const user = asRecord$17(rowsOf(json)[0]);
			return {
		...
		*/
		function YzjLoginBanner(props) {
			...
			const probe = async (afterLogin) => {
				...
				const rec = asRecord$16(result.value);
		...
		*/
		...
		function parseNavRooms(value) {
			return asArray$11(asRecord$15(value).rooms).flatMap((row) => {
				const rec = asRecord$15(row);
		...
		*/
		function MessageBody({ message, onOpenImage, onOpenPdf, inject }) {
			...
			const param = asRecord$14(message.param);
			if (msgType === "other" && asString$11(param.title) === "" && asRecord$14(param.interactiveCard).cardJson === void 0) return (0, react_jsx_runtime.jsx)("span", {
			...
			if (msgType === "other") {
				const cardJson = asString$11(asRecord$14(param.interactiveCard).cardJson);
				const face = cardJson === "" ? {
					title: "",
					image: "",
					actionTitle: "",
			...
			if (msgType === "richText") {
				const desc = asArray$10(param.desc);
				const images = [];
				const bolds = [];
				for (const raw of desc) {
					const seg = asRecord$14(raw);
		...
		function YzjTopicLens(props) {
			...
			return (0, react_jsx_runtime.jsxs)("aside", {
				...
				children: [
					...
					showAnchor && (0, react_jsx_runtime.jsxs)("button", {
						type: "button",
					...
					(0, react_jsx_runtime.jsxs)("aside", {
						...
						children: [
							(0, react_jsx_runtime.jsxs)("div", {
								...
								children: [(0, react_jsx_runtime.jsx)("b", { children: "我的推进" }), (0, react_jsx_runtime.jsx)("span", { children: "当前需要关注的事项" })]
							}),
							...
							(0, react_jsx_runtime.jsxs)("header", {
								...
								children: [
									(0, react_jsx_runtime.jsxs)("div", {
										...
										children: [(0, react_jsx_runtime.jsx)("span", { children: asString$5(detail.item.advanceId) }), (0, react_jsx_runtime.jsx)("span", {
										...
										(0, react_jsx_runtime.jsxs)("section", {
											...
											children: [
												...
												stage === "decision-needed" && (0, react_jsx_runtime.jsxs)("div", {
													...
													children: [latestDecision !== void 0 && (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)("h3", { children: asString$5(latestDecision.summary) }), asString$5(latestDecision.detail) !== "" && (0, react_jsx_runtime.jsx)("p", { children: asString$5(latestDecision.detail) })] }), (0, react_jsx_runtime.jsxs)("div", {
												...
												stage === "completed" && (0, react_jsx_runtime.jsx)("p", {
													className: advance_pane_module_css_default.quiet,
