window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SheetBody(meta) {
			const sheets = asArray$14(asRecord$20(meta.schema).sheets);
			if (sheets.length > 0) return (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: sheets.map((item, index) => {
					const table = asRecord$20(item);
					const fields = asArray$14(table.fields).map((field) => asString$12(asRecord$20(field).name)).filter((name) => name !== "");
		...
		function ImBody(meta, openPanel) {
			...
			if (messages.length > 0 && asString$12(asRecord$20(messages[0]).sendTime) !== "") return (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: messages.map((item, index) => {
					...
					const reply = asString$12(asRecord$20(message.param).replySummary);
			...
			if (groups.length > 0) return (0, react_jsx_runtime.jsx)("div", {
				...
				children: groups.map((item, index) => {
					...
					const last = asString$12(asRecord$20(group.lastMsg).content);
		...
		function TodoBody(meta, toolName) {
			...
			const link = asString$12(asRecord$20(meta.library).link);
		...
		*/
		function YzjHomeChrome(props) {
			const [kind, setKind] = (0, react.useState)("unbound");
			...
			(0, react.useEffect)(() => {
				...
				const tick = async () => {
					...
					const next = raw.kind === "room" || raw.kind === "topic" ? raw.kind : raw.bound === true ? "room" : "unbound";
					setKind(next);
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					kind === "room" ? (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)("span", { children: "群房间 · 发送 = 发进群（不叫助手）" }), (0, react_jsx_runtime.jsx)("button", {
						type: "button",
					...
					})] }) : kind === "topic" ? (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)("span", { children: "话题 · 下方发送 = 问助手。助手要发群会出确认卡。" }), roomSessionId !== "" && (0, react_jsx_runtime.jsx)("button", {
						type: "button",
		...
		function HandoffModal(props) {
			...
			(0, react.useEffect)(() => {
				props.fetchGroups(20).then((result) => {
					if (!result.ok) return;
					const rows = asArray$13(asRecord$19(result.value).list).map((item) => {
				...
				props.homeDigest(props.sessionId).then((result) => {
					if (!result.ok) return;
					const list = asArray$13(asRecord$19(result.value).candidates);
					setCandidates(list);
			...
			try {
				switch (ref.kind) {
					case "workspace": {
						...
						if (result.ok) {
							const ws = asRecord$18(result.value);
					...
					case "doc": {
						...
						if (infoResult.ok) {
							const node = asRecord$18(infoResult.value);
						...
						if (blocksResult.ok) {
							const blocksValue = asRecord$18(blocksResult.value);
							const excerpt = asArray$12(asRecord$18(blocksValue.data).blocks ?? blocksValue.blocks).slice(0, 10).map(blockText).filter((text) => text !== "").join(" ");
							if (excerpt !== "") {
								lines.push(`内容摘要：${excerpt.length > 500 ? `${excerpt.slice(0, 500)}…` : excerpt}`);
								lines.push("（内容为摘要，完整内容可用 yzj_doc_block_list / yzj_doc_get 获取）");
							}
						}
						if (infoResult.ok && asString$11(asRecord$18(infoResult.value).fileSuffix) === "dbt") {
							const sheetResult = await inject.fetchSheet(ref.id);
							if (sheetResult.ok) {
								const sheetValue = asRecord$18(sheetResult.value);
								const tableLines = asArray$12(sheetValue.sheets ?? asRecord$18(sheetValue.data).sheets).slice(0, 5).map((item) => {
									const table = asRecord$18(item);
									const fields = asArray$12(table.fields).map((field) => asString$11(asRecord$18(field).name)).filter((name) => name !== "");
					...
					case "group": {
						...
						if (result.ok) {
							const preview = [...asArray$12(asRecord$18(result.value).list)].reverse().slice(0, 6).map((item) => {
					...
					case "event": {
						...
						if (result.ok) {
							const event = asRecord$18(result.value);
					...
					case "contact": {
						...
						if (result.ok) {
							const user = asRecord$18(asArray$12(result.value)[0] ?? result.value);
							const parts = [
					...
					case "todo": {
						...
						if (result.ok) {
							const value = asRecord$18(result.value);
							const todo = asArray$12(value.todos).map(asRecord$18).find((item) => asString$11(item.todoId) === ref.id);
							if (todo !== void 0) {
								...
								const library = asRecord$18(value.library);
					...
					case "message": {
						...
						if (groupId !== "") {
							...
							if (result.ok) {
								const hit = asArray$12(asRecord$18(result.value).list).find((item) => asString$11(asRecord$18(item).msgId) === ref.id);
								if (hit !== void 0) {
									const message = asRecord$18(hit);
									const body = asString$11(message.content);
		...
		function ensureWarm(cache, inject) {
			...
			cache.warm = Promise.all([inject.fetchWorkspaces().then((result) => {
				...
				if (result.ok) cache.groups = asArray$11(asRecord$17(result.value).list);
			}).catch(() => {})]).then(() => {
				const roots = cache.workspaces.slice(0, 3);
				return Promise.all(roots.map((workspace) => inject.fetchDocs(asString$10(asRecord$17(workspace).id)).then((result) => {
		...
		*/
		...
		function speakersOf(value) {
			const items = asRecord$16(value).items;
			if (!Array.isArray(items)) return [];
			const byId = /* @__PURE__ */ new Map();
			for (const item of items) {
				const row = asRecord$16(asRecord$16(item).entry);
			...
			const pickFile = (kind, file) => {
				...
				reader.onload = () => {
					...
					props.uploadFile?.(file.name, base64, file.size).then(async (result) => {
						...
						const payload = asRecord$16(result.value);
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("p", {
						...
						children: "这里发送只有一个意思：以本人身份发进群（公开，无确认卡）。@ 同事须是本群发过言的人。要用助手，点消息旁的「交给助手」。"
					}),
					error !== "" && (0, react_jsx_runtime.jsx)("p", {
						role: "alert",
						children: error
		...
		*/
		function syncYzjViewRing(kind) {
			...
			if (kind === "room") {
				...
				if (tablist !== void 0 && tablist !== null) tablist.hidden = true;
				if (roomTab !== void 0) roomTab.hidden = false;
				return;
			}
			if (tablist !== void 0 && tablist !== null) tablist.hidden = false;
		...
		function YzjSessionShell(props) {
			...
			if (kind === "topic") {
				...
				return (0, react_jsx_runtime.jsxs)("button", {
					...
					children: [(0, react_jsx_runtime.jsxs)("span", { children: ["群消息锚点", summary === "" ? "" : `：${summary}`] }), (0, react_jsx_runtime.jsx)("span", {
		...
		*/
		...
		function rowsOf(json) {
			...
			const record = asRecord$14(json);
			if (Array.isArray(record.list)) return record.list;
			if (Array.isArray(record.data)) return record.data;
			if (typeof record.data === "object" && record.data !== null) {
				const inner = asRecord$14(record.data);
		...
		function parseContactUser(json) {
			const user = asRecord$14(rowsOf(json)[0]);
			return {
		...
		*/
		...
		function parseNavRooms(value) {
			return asArray$10(asRecord$13(value).rooms).flatMap((row) => {
				const rec = asRecord$13(row);
		...
		* as the floating-panel 会话 list.
		*/
		function YzjConvList(props) {
			const [bound, setBound] = (0, react.useState)([]);
			...
			return (0, react_jsx_runtime.jsxs)("nav", {
				...
				children: [
					...
					rows.length === 0 && error === "" && (0, react_jsx_runtime.jsx)("p", {
						className: home_module_css_default.convListHint,
						children: "还没有最近会话。点侧栏「对话」或从悬浮球「会话」挑一个。"
					}),
					(0, react_jsx_runtime.jsx)("div", {
							...
							return (0, react_jsx_runtime.jsxs)("button", {
								...
								children: [(0, react_jsx_runtime.jsx)("span", {
									...
									children: [(0, react_jsx_runtime.jsxs)("span", {
										...
										}), row.topicCount > 0 && (0, react_jsx_runtime.jsx)("span", {
											className: home_module_css_default.convDot,
											title: `${row.topicCount} 个话题`
										})]
									})]
								})]
							}, row.groupId);
					...
					more && (0, react_jsx_runtime.jsx)("button", {
						type: "button",
		...
		* the first recent conversation); other domains open the floating panel
		* (P0). Memory is a placeholder noting the local vault.
		*/
		const DOCK = [
			{
				id: "chat",
		...
		function robotLine(value) {
			const channels = asArray$9(asRecord$12(value).channels);
			if (channels.length === 0) return "机器人通道 · 未配置";
			const connected = channels.filter((row) => asRecord$12(row).connected === true).length;
		...
		*/
		function YzjYunzhijiaDock(props) {
			...
			const onEntry = (id) => {
				...
				setHint("记忆是本地 vault，不出本机。P0 请到 设置 → 云之家 打开。");
			};
			return (0, react_jsx_runtime.jsxs)("nav", {
		...
		*/
		function MessageBody({ message, onOpenImage, onOpenPdf, inject }) {
			...
			const param = asRecord$11(message.param);
			if (msgType === "other" && asString$8(param.title) === "" && asRecord$11(param.interactiveCard).cardJson === void 0) return (0, react_jsx_runtime.jsx)("span", {
			...
			if (msgType === "other") {
				const cardJson = asString$8(asRecord$11(param.interactiveCard).cardJson);
				const face = cardJson === "" ? {
					title: "",
					image: "",
					actionTitle: "",
		...
		*/
		function YzjTopicDrawer(props) {
			...
			const title = lens === void 0 ? "话题" : topicNavLabel(props.groupName, lens.title);
			if (props.lensSessionId !== void 0) {
				const origin = lens?.originText ?? "";
				const who = lens?.originWho ?? "";
				const when = clock$1(lens?.originTime);
				...
				return (0, react_jsx_runtime.jsxs)("aside", {
					...
					children: [
						...
						rootMsgId !== void 0 && rootMsgId !== "" && (0, react_jsx_runtime.jsxs)("button", {
							type: "button",
						...
						(0, react_jsx_runtime.jsx)("div", {
							...
							children: (0, react_jsx_runtime.jsx)("p", {
								...
								children: "透镜只作对照。问助手、工具卡、确认卡都在原生会话里。"
							})
						}),
		...
		*/
		function YzjFusedView(props) {
				...
				const load = async (backfill) => {
					if (backfill) {
						...
						if (!cancelled && stats.ok) {
							if (asRecord$10(stats.value).more === false) setMore(false);
						}
					}
					const result = await props.homeFused(props.sessionId);
					if (cancelled) return;
					...
					setError("");
					const raw = asRecord$10(result.value);
					...
					if (props.fetchContact !== void 0) {
						const found = await resolveSenders(items.flatMap((item) => item.kind === "im" && item.entry.fromOpenId !== void 0 ? [item.entry.fromOpenId] : []), { fetchContact: props.fetchContact });
			...
			const openTopic = async (entry) => {
				...
				const sessionId = typeof asRecord$10(result.value).sessionId === "string" ? asRecord$10(result.value).sessionId : "";
				if (sessionId === "") return;
				setOptimistic((prev) => {
			...
			const loadOlder = async () => {
				...
				const result = await props.homeFused(props.sessionId);
				if (!result.ok) return;
				const next = asRecord$10(result.value);
			...
			if (!value.bound || value.kind === "unbound") return (0, react_jsx_runtime.jsx)("div", {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					isGroup && (0, react_jsx_runtime.jsx)("div", {
									...
									children: [!mine && (node.merged ? (0, react_jsx_runtime.jsx)("span", {
										...
										children: [
											...
											(0, react_jsx_runtime.jsxs)("span", {
												...
												children: [
													!hideFileBody && (0, react_jsx_runtime.jsx)(MessageBody, {
		...
		*/
		function YzjRoomShell(props) {
			const [activeGroupId, setActiveGroupId] = (0, react.useState)("");
			(0, react.useEffect)(() => {
				let cancelled = false;
				const load = async () => {
					const result = await props.homeFused(props.sessionId);
					if (cancelled || !result.ok) return;
					const binding = asRecord$9(asRecord$9(result.value).binding);
