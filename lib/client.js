	...
	factory: (require) => {
		...
		function MemoryBody(meta, toolName) {
			...
			if (toolName === "memory_observe") rowsOut.push(row$1(meta.duplicate === true ? "这条已经在记忆里" : "已记入观察草稿区", `open ${typeof meta.openCount === "number" ? meta.openCount : 0}/${typeof meta.capacity === "number" ? meta.capacity : 0}，等待 dream 固化`, "obs"));
				...
				const sections = asArray$11(meta.sections);
				const entities = asArray$11(meta.entities);
				const observations = asArray$11(meta.observations);
				...
				const hits = asArray$11(meta.hits);
				...
				for (const [index, item] of asArray$11(meta.results).slice(0, 5).entries()) {
