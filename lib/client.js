window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function DocBody(meta, openPanel, listKind) {
			const list = asArray$10(meta.list);
			if (list.length > 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: list.map((item, index) => {
					const node = asRecord$11(item);
					const name = asString$9(node.name) !== "" ? asString$9(node.name) : asString$9(node.title);
					const kind = asNumber(node.visibility) === 2 ? "个人" : "";
					const url = asString$9(node.openWebUrl);
					const id = asString$9(node.id);
			...
			const record = asRecord$11(meta.record);
			const title = asString$9(record.title) || asString$9(record.name);
			const link = asString$9(record.openWebUrl);
			const id = asString$9(record.id);
			if (title !== "") {
				const suffix = asString$9(record.fileSuffix);
				...
				const sub = [
					...
					perm ?? "",
					asString$9(record.creatorName),
					asString$9(record.updateTime).slice(0, 10)
		...
		function BlockBody(meta) {
			const blocks = asArray$10(meta.list);
			if (blocks.length === 0) return null;
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: blocks.map((item, index) => {
					const block = asRecord$11(item);
					const type = asString$9(block.type);
					const content = asString$9(block.content).replace(/\s+/g, " ").slice(0, 80);
		...
		function SheetBody(meta) {
			const sheets = asArray$10(asRecord$11(meta.schema).sheets);
			if (sheets.length > 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: sheets.map((item, index) => {
					const table = asRecord$11(item);
					const fields = asArray$10(table.fields).map((field) => asString$9(asRecord$11(field).name)).filter((name) => name !== "");
					return row$1(asString$9(table.name), fields.length === 0 ? "" : `字段：${fields.join(" / ")}`, `t${index}`);
				})
			});
			const table = asRecord$11(meta.table);
			if (asString$9(table.name) !== "") {
				const fields = asArray$10(table.fields).map((field) => asString$9(asRecord$11(field).name)).filter((name) => name !== "");
				return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: cards_module_css_default.rows,
					children: row$1(asString$9(table.name), fields.length === 0 ? "" : `字段：${fields.join(" / ")}`, "t")
				});
			}
			const records = asArray$10(meta.list);
			if (records.length > 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: records.map((item, index) => {
					const record = asRecord$11(item);
					const fields = asRecord$11(record.fieldsValue ?? record.fields);
		...
		function CalendarBody(meta, openPanel) {
			const events = asArray$10(meta.list);
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: events.map((item, index) => {
					const event = asRecord$11(item);
					const start = clock(event.startDate);
					const end = clock(event.endDate);
					const time = start === "" ? "" : `${start}${end === "" ? "" : ` → ${end}`}`;
					const person = asString$9(event.personName);
					const id = asString$9(event.id);
					const startMs = typeof event.startDate === "number" ? event.startDate : 0;
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: cards_module_css_default.rowWrap,
						children: [row$1(asString$9(event.title), [time, person].filter((part) => part !== "").join(" · "), `e${index}`), id !== "" && startMs > 0 && jumpRow("查看", () => openPanel({
							kind: "event",
							event: {
								id,
								startDate: startMs,
								title: asString$9(event.title)
							}
						}), `j${index}`)]
					}, `e${index}`);
				})
		...
		function ImBody(meta, openPanel) {
			const messages = asArray$10(meta.list);
			if (messages.length > 0 && asString$9(asRecord$11(messages[0]).sendTime) !== "") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: messages.map((item, index) => {
					const message = asRecord$11(item);
					const time = asString$9(message.sendTime).slice(5, 16);
					const content = asString$9(message.content);
					const reply = asString$9(asRecord$11(message.param).replySummary);
			...
			if (groups.length > 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: groups.map((item, index) => {
					...
					const last = asString$9(asRecord$11(group.lastMsg).content);
					const groupId = asString$9(group.groupId);
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: cards_module_css_default.rowWrap,
						children: [row$1(asString$9(group.groupName), [unread !== void 0 && unread > 0 ? `未读 ${unread}` : "", last.replace(/\s+/g, " ").slice(0, 40)].filter((part) => part !== "").join(" · "), `g${index}`), groupId !== "" && jumpRow("查看", () => openPanel({
							kind: "group",
							groupId
						}), `j${index}`)]
					}, `g${index}`);
		...
		function TodoBody(meta, toolName) {
			...
			if (toolName === "yzj_todo_list") {
				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: cards_module_css_default.rows,
					children: list.map((item, index) => {
						const todo = asRecord$11(item);
						const tags = asArray$10(todo.tags).filter((tag) => typeof tag === "string");
						const overdue = todo.overdue === true;
						const status = statusLabel[asString$9(todo.status)] ?? asString$9(todo.status);
						const sub = [
							asString$9(todo.ddl) === "" ? "" : `${overdue ? "逾期 " : ""}DDL ${asString$9(todo.ddl)}`,
							asString$9(todo.assignee) === "" ? "" : `@${asString$9(todo.assignee)}`,
							tags.length === 0 ? "" : tags.map((tag) => `#${tag}`).join(" ")
						].filter((part) => part !== "").join(" · ");
						return row$1(asString$9(todo.title) === "" ? "(无标题)" : asString$9(todo.title), [status, sub].filter((part) => part !== "").join(" · "), `t${index}`);
					})
				});
			}
			const rowsOut = [];
			const title = asString$9(meta.title);
			const tags = asArray$10(meta.tags).filter((tag) => typeof tag === "string");
			const sub = [asString$9(meta.ddl) === "" ? "" : `DDL ${asString$9(meta.ddl)}`, tags.length === 0 ? "" : tags.map((tag) => `#${tag}`).join(" ")].filter((part) => part !== "").join(" · ");
			if (toolName === "yzj_todo_create") rowsOut.push(row$1(meta.idempotentHit === true ? `已存在：${title}` : `已创建：${title}`, sub, "c"));
				...
				const changes = asArray$10(meta.changes).filter((change) => typeof change === "string");
				rowsOut.push(row$1(`已更新：${title}`, changes.join("；"), "u"));
			}
			const link = asString$9(asRecord$11(meta.library).link);
		...
		function MemoryBody(meta, toolName) {
			...
			if (toolName === "memory_observe") rowsOut.push(row$1(meta.duplicate === true ? "这条已经在记忆里" : "已记入观察草稿区", `open ${typeof meta.openCount === "number" ? meta.openCount : 0}/${typeof meta.capacity === "number" ? meta.capacity : 0}，等待 dream 固化`, "obs"));
				...
				const sections = asArray$10(meta.sections);
				const entities = asArray$10(meta.entities);
				const observations = asArray$10(meta.observations);
				...
				for (const [index, item] of sections.slice(0, 5).entries()) {
					const section = asRecord$11(item);
					rowsOut.push(row$1(`段 · ${asString$9(section.title) || asString$9(section.name)}`, asString$9(section.excerpt), `s${index}`));
				}
				if (sections.length > 5) rowsOut.push(row$1(`…其余 ${sections.length - 5} 段`, "", "smore"));
			} else if (toolName === "memory_search") {
				const hits = asArray$10(meta.hits);
				if (hits.length === 0) rowsOut.push(row$1("无匹配记忆", "", "empty"));
				for (const [index, item] of hits.slice(0, 8).entries()) {
					const hit = asRecord$11(item);
					rowsOut.push(row$1(`${{
						section: "段",
						entity: "实体",
						observation: "观察"
					}[asString$9(hit.kind)] ?? asString$9(hit.kind)} · ${asString$9(hit.ref)}`, asString$9(hit.line), `h${index}`));
				}
				if (hits.length > 8) rowsOut.push(row$1(`…其余 ${hits.length - 8} 条命中`, "", "hmore"));
			} else if (toolName === "memory_dream_apply") {
				const counts = asRecord$11(meta.counts);
				const parts = [
					"promoted",
					"dropped",
					"sectionsWritten",
				...
				rowsOut.push(row$1(`固化完成 ${asString$9(meta.logId)}`, parts, "dream"));
				for (const [index, item] of asArray$10(meta.results).slice(0, 5).entries()) {
					const result = asRecord$11(item);
					rowsOut.push(row$1(`${result.ok === true ? "✓" : "✗"} ${asString$9(result.decision)} — ${asString$9(result.detail)}`, asString$9(result.reason), `r${index}`));
				}
			}
		...
		/** Contact-domain body (whoami / search / details). */ function ContactBody(meta) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: users.map((item, index) => {
					...
					const sub = [asString$9(user.department ?? user.fulldepartment), asString$9(user.jobTitle)].filter((part) => part !== "").join(" · ");
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: cards_module_css_default.row,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: cards_module_css_default.rowTitle,
		...
		function ActionBody(meta, toolName) {
			const rowsOut = [];
			const link = asString$9(meta.link);
			const url = asString$9(meta.url);
			const output = asString$9(meta.output);
			const recordIds = asArray$10(meta.recordIds);
		...
		*/
		function YzjToolCard({ toolName, block, openPanel }) {
			...
			const meta = asRecord$11(block.meta);
			let body = null;
			if (toolName === "yzj_doc_block_list") body = BlockBody(meta);
			else if (toolName === "yzj_calendar_event_participants") body = listRows(asArray$10(meta.list), ["name"], ["jobTitle", "department"]);
			else if (toolName === "yzj_calendar_room_find") body = listRows(asArray$10(meta.list), ["name", "title"], ["capacity", "floor"]);
		...
		function blockText(node) {
			const record = asRecord$10(node);
			const parts = [];
			const own = asString$8(record.content);
			if (own !== "") parts.push(own);
			const childArray = asArray$9(record.childNodes ?? record.children);
			...
			try {
				switch (ref.kind) {
					case "workspace": {
						...
						if (result.ok) {
							const ws = asRecord$10(result.value);
							lines.push(`类型：${asString$8(ws.bizType) === "" ? "知识库" : asString$8(ws.bizType)} · 文档 ${typeof ws.docCount === "number" ? ws.docCount : "?"} 篇 · 成员 ${typeof ws.memberCount === "number" ? ws.memberCount : "?"} 人`);
							if (asString$8(ws.description) !== "") lines.push(`简介：${asString$8(ws.description)}`);
						}
						break;
					}
					case "doc": {
						...
						if (infoResult.ok) {
							const node = asRecord$10(infoResult.value);
							const suffix = asString$8(node.fileSuffix);
							lines.push(`类型：${suffix === "dbt" ? "多维表格" : "在线文档"} · 更新 ${asString$8(node.updateTime).slice(0, 10)} · 创建人 ${asString$8(node.creatorName) === "" ? "未知" : asString$8(node.creatorName)}`);
							const link = asString$8(node.openWebUrl);
							if (link !== "") lines.push(`链接：${link}`);
						}
						if (blocksResult.ok) {
							const blocksValue = asRecord$10(blocksResult.value);
							const excerpt = asArray$9(asRecord$10(blocksValue.data).blocks ?? blocksValue.blocks).slice(0, 10).map(blockText).filter((text) => text !== "").join(" ");
							if (excerpt !== "") {
								lines.push(`内容摘要：${excerpt.length > 500 ? `${excerpt.slice(0, 500)}…` : excerpt}`);
								lines.push("（内容为摘要，完整内容可用 yzj_doc_block_list / yzj_doc_get 获取）");
							}
						}
						if (infoResult.ok && asString$8(asRecord$10(infoResult.value).fileSuffix) === "dbt") {
							const sheetResult = await inject.fetchSheet(ref.id);
							if (sheetResult.ok) {
								const sheetValue = asRecord$10(sheetResult.value);
								const tableLines = asArray$9(sheetValue.sheets ?? asRecord$10(sheetValue.data).sheets).slice(0, 5).map((item) => {
									const table = asRecord$10(item);
									const fields = asArray$9(table.fields).map((field) => asString$8(asRecord$10(field).name)).filter((name) => name !== "");
									return `- ${asString$8(table.name)}${fields.length === 0 ? "" : `：${fields.join(" / ")}`}`;
								});
					...
					case "group": {
						...
						if (result.ok) {
							const preview = [...asArray$9(asRecord$10(result.value).list)].reverse().slice(0, 6).map((item) => {
								const message = asRecord$10(item);
								const time = asString$8(message.sendTime).slice(5, 16);
								const body = asString$8(message.content);
					...
					case "event": {
						...
						if (result.ok) {
							const event = asRecord$10(result.value);
							const span = [clock(event.startDate), clock(event.endDate)].filter((part) => part !== "").join(" → ");
							lines.push(`时间：${span === "" ? "未知" : span}`);
							if (asString$8(event.personName) !== "") lines.push(`组织者：${asString$8(event.personName)}`);
							if (asString$8(event.content) !== "") lines.push(`描述：${asString$8(event.content).slice(0, 200)}`);
						}
						break;
					}
					case "contact": {
						...
						if (result.ok) {
							const user = asRecord$10(asArray$9(result.value)[0] ?? result.value);
							const parts = [
								asString$8(user.department),
								asString$8(user.jobTitle),
								asString$8(user.jobNo) === "" ? "" : `工号 ${asString$8(user.jobNo)}`
							];
					...
					case "todo": {
						...
						if (result.ok) {
							...
							const todo = asArray$9(value.todos).map(asRecord$10).find((item) => asString$8(item.todoId) === ref.id);
							if (todo !== void 0) {
								const parts = [`状态：${asString$8(todo.status)}`];
								if (asString$8(todo.ddl) !== "") parts.push(`DDL：${asString$8(todo.ddl)}${todo.overdue === true ? "（已逾期）" : ""}`);
								if (asString$8(todo.priority) !== "") parts.push(`优先级：${asString$8(todo.priority)}`);
								if (asTagsOf(todo.tags).length > 0) parts.push(`标签：${asTagsOf(todo.tags).map((tag) => `#${tag}`).join(" ")}`);
								if (asString$8(todo.assignee) !== "") parts.push(`负责人：${asString$8(todo.assignee)}`);
								lines.push(parts.join(" · "));
								const log = asString$8(todo.log);
								if (log !== "") {
									const tail = log.split("\n").slice(-3);
									lines.push(`推进日志（最近）：\n${tail.map((line) => `- ${line}`).join("\n")}`);
								}
								const library = asRecord$10(value.library);
								if (asString$8(library.link) !== "") lines.push(`任务库：${asString$8(library.link)}`);
					...
					case "message": {
						const groupId = asString$8(ref.group);
						if (groupId !== "") {
							...
							if (result.ok) {
								const hit = asArray$9(asRecord$10(result.value).list).find((item) => asString$8(asRecord$10(item).msgId) === ref.id);
								if (hit !== void 0) {
									const message = asRecord$10(hit);
									const body = asString$8(message.content);
									const from = asString$8(message.fromOpenId);
									lines.push(`发送人：${from === "" ? "(未知)" : from}`);
									lines.push(`原文：${body === "" ? `(${asString$8(message.msgType) === "" ? "消息" : asString$8(message.msgType)})` : body}`);
								} else lines.push(`内容（快照，原文可能已变）：${ref.title}`);
							} else lines.push(`内容（快照，原文可能已变）：${ref.title}`);
						} else lines.push(`内容（快照，原文可能已变）：${ref.title}`);
						if (asString$8(ref.sub) !== "") lines.push(`时间：${asString$8(ref.sub)}`);
						break;
					}
				}
			} catch {}
		...
		function ensureWarm(cache, inject) {
			...
			cache.warm = Promise.all([inject.fetchWorkspaces().then((result) => {
				if (result.ok) cache.workspaces = asArray$8(result.value);
			}).catch(() => {}), inject.fetchGroups(20).then((result) => {
				if (result.ok) cache.groups = asArray$8(asRecord$9(result.value).list);
			}).catch(() => {})]).then(() => {
				const roots = cache.workspaces.slice(0, 3);
				return Promise.all(roots.map((workspace) => inject.fetchDocs(asString$7(asRecord$9(workspace).id)).then((result) => {
					if (result.ok) cache.docs = [...cache.docs, ...asArray$8(result.value)];
				}).catch(() => {})));
			}).then(() => {});
			return cache.warm;
		}
		...
		function contactCandidates(cache, query, inject) {
			...
			return inject.fetchSearch(q).then((result) => {
				const out = [];
				if (result.ok) for (const item of asArray$8(result.value)) {
					const user = asRecord$9(item);
					const name = asString$7(user.name);
					if (name === "") continue;
					const sub = [asString$7(user.department), asString$7(user.jobTitle)].filter((part) => part !== "").join(" · ");
					pushCandidate(cache, out, name, `👤 ${sub === "" ? "联系人" : sub}（仅你有权查看的范围）`, KIND_ICON.contact, {
						kind: "contact",
						id: asString$7(user.oId ?? user.openId),
						title: name
					});
				}
				return out;
		...
		function groupCandidates(cache, query) {
			...
			for (const item of cache.groups) {
				...
				const name = asString$7(group.groupName);
				if (name === "") continue;
		...
		function docCandidates(cache, query) {
			...
			for (const item of cache.docs) {
				...
				const title = asString$7(node.title);
				if (title === "") continue;
				if (q !== "" && !title.toLowerCase().includes(q)) continue;
				const kindText = asString$7(node.fileSuffix) === "dbt" ? "多维表格" : "文档";
				const updated = asString$7(node.updateTime).slice(0, 10);
				pushCandidate(cache, out, title, `📄 ${kindText}${updated === "" ? "" : ` · 更新 ${updated}`}`, KIND_ICON.doc, {
					kind: "doc",
					id: asString$7(node.id),
					title
				});
			}
			return out;
		...
		*/
		...
		const tagId$4 = "@dsh-yzj/ui-yzj/todo-pane.module.css";
		...
		*  without hand-editing host config. */
								...
								return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(StatusDot, {
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											...
											children: [
												asString$5(todo.ddl) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
									...
									children: [
										...
										asString$5(todo.assignee) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											className: todo_pane_module_css_default.detailLine,
											children: ["负责人：", asString$5(todo.assignee)]
										}),
										asString$5(todo.log) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		...
		var panel_module_css_default = {
			...
			"dropOverlayHint": "-xzL-q_dropOverlayHint",
		...
		*/
		function MessageBody({ message, onOpenImage, onOpenPdf, inject }) {
			...
			if (msgType === "richText") {
				const desc = asArray$5(param.desc);
				const images = [];
				const bolds = [];
		...
		function YzjFloatBall(props) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					role: "group",
					"aria-label": "云之家快捷入口",
					children: DOCK_ITEMS.map((item) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						type: "button",
						...
						children: [
							...
							item.key === "chat" && unreadTotal > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(YzjCloudIcon, { size: 22 }), unreadTotal > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
			...
			*  current month reselects today; other months clear the selection. */
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: state.groupId === "" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										ref: listRef,
										children: [
											state.messages.length === 0 && !state.loading && state.error === "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
												className: panel_module_css_default.empty,
												children: "暂无消息"
											}),
											state.messagesMore && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
												...
												disabled: state.loading,
												children: state.loading ? "加载中…" : "加载更早消息"
											}),
										...
										children: [
											...
											/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
												...
												children: [
													...
													atMenu !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
														className: panel_module_css_default.atMenu,
														role: "listbox",
														"aria-label": "提及成员",
														children: [
															atMatches.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
																className: panel_module_css_default.atHint,
																children: atCandidates.length === 0 ? "本会话暂无已知成员（发过言才可 @）" : "无匹配成员"
															}),
															atMatches.map((candidate) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
																type: "button",
																role: "option",
																...
																}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: candidate.name })]
															}, candidate.openId)),
															/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
																type: "button",
																className: panel_module_css_default.atItem,
																...
																children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
																	className: panel_module_css_default.atGlyph,
																	children: "@"
																}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "所有人（@all）" })]
															})
														]
													}),
													/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											...
											/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
												...
												children: [
													/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
														...
														children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconImage14, {})
													}),
													/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
														type: "button",
														className: panel_module_css_default.toolButton,
														...
														children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconClip14, {})
													}),
													/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
														type: "button",
														className: panel_module_css_default.toolButton,
														...
														children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconSmile14, {})
													}),
													uploading && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					...
					lightbox !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: panel_module_css_default.lightbox,
						role: "presentation",
						onClick: () => setLightbox(null),
						children: lightbox.kind === "pdf" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("embed", {
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:D:\dev\dsh-yzj\packages\ui-yzj\src\client\memory-pane.module.css.mjs
		const css$1 = ".F3REFG_body{flex-direction:column;flex:1;gap:10px;min-height:0;padding:10px 12px 12px;display:flex;overflow-y:auto}.F3REFG_stats{flex-wrap:wrap;align-items:center;gap:8px;display:flex}.F3REFG_statsMain{color:var(--dsw-alias-label-primary);font-size:13px;font-weight:600}.F3REFG_statsMeta{color:var(--dsw-alias-label-tertiary);font-size:11.5px}.F3REFG_refresh{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-secondary);cursor:pointer;border-radius:7px;margin-left:auto;padding:6px 10px;font-size:12px;line-height:1}.F3REFG_refresh:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.F3REFG_error{color:var(--dsw-static-red-400);margin:0;font-size:12px;line-height:16px}.F3REFG_dream{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:10px;flex-direction:column;gap:8px;padding:10px;display:flex}.F3REFG_dreamHead{flex-wrap:wrap;align-items:center;gap:8px;display:flex}.F3REFG_dreamTitle{color:var(--dsw-alias-label-primary);font-size:13px;font-weight:600}.F3REFG_switchOn,.F3REFG_switchOff{cursor:pointer;border:1px solid #0000;border-radius:999px;padding:6px 12px;font-size:12px;line-height:1}.F3REFG_switchOn{background:var(--dsw-static-deepseek-100);border-color:var(--dsw-static-deepseek-500);color:var(--dsw-static-deepseek-600)}.F3REFG_switchOff{background:var(--dsw-alias-bg-layer-2);border-color:var(--dsw-alias-border-l2);color:var(--dsw-alias-label-tertiary)}.F3REFG_dreamRun{background:var(--dsw-alias-button-info-fill);color:var(--dsw-alias-label-primary-foreground);cursor:pointer;border:none;border-radius:7px;padding:7px 12px;font-size:12px;line-height:1}.F3REFG_dreamRun:hover{background:var(--dsw-alias-button-info-hover)}.F3REFG_dreamRunOff{opacity:.45;cursor:default}.F3REFG_dreamRow{flex-wrap:wrap;align-items:center;gap:10px;display:flex}.F3REFG_dreamLabel{color:var(--dsw-alias-label-secondary);align-items:center;gap:6px;font-size:12px;display:inline-flex}.F3REFG_dreamHint{color:var(--dsw-alias-label-caption);font-size:11px}.F3REFG_timeInput{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);border-radius:6px;padding:4px 6px;font-family:inherit;font-size:12px}.F3REFG_picker{align-items:center;gap:4px;display:inline-flex}.F3REFG_pickerSelect{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);border-radius:6px;max-width:180px;padding:4px 6px;font-family:inherit;font-size:12px}.F3REFG_dreamLast{color:var(--dsw-alias-label-tertiary);word-break:break-word;margin:0;font-size:11.5px;line-height:16px}.F3REFG_dreamNote{color:var(--dsw-alias-label-secondary);word-break:break-word;margin:0;font-size:11.5px;line-height:16px}.F3REFG_quick{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:10px;flex-wrap:wrap;align-items:flex-start;gap:8px;padding:8px;transition:border-color .15s;display:flex}.F3REFG_quick:focus-within{border-color:var(--dsw-static-deepseek-500)}.F3REFG_quickInput{min-width:200px;color:var(--dsw-alias-label-primary);resize:vertical;background:0 0;border:none;outline:none;flex:1;font-family:inherit;font-size:13px;line-height:18px}.F3REFG_quickInput::placeholder{color:var(--dsw-alias-label-caption)}.F3REFG_quickAdd{background:var(--dsw-alias-button-info-fill);color:var(--dsw-alias-label-primary-foreground);cursor:pointer;border:none;border-radius:7px;flex-shrink:0;padding:8px 12px;font-size:12px;line-height:1}.F3REFG_quickAdd:hover{background:var(--dsw-alias-button-info-hover)}.F3REFG_quickAddOff{opacity:.45;cursor:default}.F3REFG_durableWrap{color:var(--dsw-alias-label-secondary);cursor:pointer;user-select:none;flex-shrink:0;align-items:center;gap:4px;font-size:12px;display:inline-flex}.F3REFG_durableCheck{accent-color:var(--dsw-static-deepseek-500);cursor:pointer}.F3REFG_quickNote{color:var(--dsw-alias-label-tertiary);flex-basis:100%;font-size:11.5px;line-height:15px}.F3REFG_list{flex-direction:column;flex:1;gap:8px;min-height:0;display:flex}.F3REFG_groupTitle{color:var(--dsw-alias-label-secondary);margin:6px 0 0;font-size:12px;font-weight:600}.F3REFG_hint{color:var(--dsw-alias-label-tertiary);margin:0;font-size:12px;line-height:17px}.F3REFG_items{flex-direction:column;gap:4px;margin:0;padding:0;list-style:none;display:flex}.F3REFG_item{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:9px;overflow:hidden}.F3REFG_itemHead{text-align:left;cursor:pointer;background:0 0;border:none;align-items:baseline;gap:7px;width:100%;padding:7px 9px;font-family:inherit;display:flex}.F3REFG_itemHead:hover{background:var(--dsw-alias-interactive-bg-hover)}.F3REFG_itemCaret{color:var(--dsw-alias-label-caption);flex-shrink:0;font-size:10px}.F3REFG_itemTitle{min-width:0;color:var(--dsw-alias-label-primary);word-break:break-word;flex:1;font-size:12.5px;line-height:17px}.F3REFG_itemMeta{color:var(--dsw-alias-label-tertiary);flex-shrink:0;font-size:11px}.F3REFG_itemBody{border-top:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);white-space:pre-wrap;word-break:break-word;padding:8px 10px;font-size:12px;line-height:17px}.F3REFG_logToggle{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-secondary);cursor:pointer;border-radius:7px;align-self:flex-start;padding:6px 10px;font-size:12px;line-height:1}.F3REFG_logToggle:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.F3REFG_logBody{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);white-space:pre-wrap;word-break:break-word;border-radius:9px;max-height:260px;margin:0;padding:8px 10px;font-size:11.5px;line-height:16px;overflow-y:auto}";
		const tagId$1 = "@dsh-yzj/ui-yzj/memory-pane.module.css";
		...
		var memory_pane_module_css_default = {
			"statsMeta": "F3REFG_statsMeta",
			"dreamTitle": "F3REFG_dreamTitle",
			"timeInput": "F3REFG_timeInput",
			"dreamRun": "F3REFG_dreamRun",
			"items": "F3REFG_items",
			"switchOff": "F3REFG_switchOff",
			"quick": "F3REFG_quick",
			"item": "F3REFG_item",
			"quickNote": "F3REFG_quickNote",
			"body": "F3REFG_body",
			"statsMain": "F3REFG_statsMain",
			"picker": "F3REFG_picker",
			"dreamLast": "F3REFG_dreamLast",
			"switchOn": "F3REFG_switchOn",
			"dreamRow": "F3REFG_dreamRow",
			"quickAdd": "F3REFG_quickAdd",
			"pickerSelect": "F3REFG_pickerSelect",
			"groupTitle": "F3REFG_groupTitle",
			"quickAddOff": "F3REFG_quickAddOff",
			"itemTitle": "F3REFG_itemTitle",
			"logToggle": "F3REFG_logToggle",
			"logBody": "F3REFG_logBody",
			"hint": "F3REFG_hint",
			"dreamHint": "F3REFG_dreamHint",
			"stats": "F3REFG_stats",
			"dreamNote": "F3REFG_dreamNote",
			"itemBody": "F3REFG_itemBody",
			"itemCaret": "F3REFG_itemCaret",
			"durableWrap": "F3REFG_durableWrap",
			"dreamHead": "F3REFG_dreamHead",
			"dreamLabel": "F3REFG_dreamLabel",
			"itemHead": "F3REFG_itemHead",
			"dreamRunOff": "F3REFG_dreamRunOff",
			"list": "F3REFG_list",
			"itemMeta": "F3REFG_itemMeta",
			"durableCheck": "F3REFG_durableCheck",
			"quickInput": "F3REFG_quickInput",
			"error": "F3REFG_error",
			"dream": "F3REFG_dream",
			"refresh": "F3REFG_refresh"
		};
		//#endregion
		...
		*/
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
						...
						children: [
							...
							asString$2(dream.lastNote) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
		...
		const tagId = "@dsh-yzj/ui-yzj/settings-section.module.css";
		...
		function ArgBody({ record, names }) {
			...
			switch (record.domain) {
				case "im": {
					...
					if (str("content") !== "") rows.push(/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
