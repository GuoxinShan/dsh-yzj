window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function DocBody(meta, openPanel, listKind) {
			const list = asArray$8(meta.list);
			...
			const record = asRecord$17(meta.record);
		...
		function BlockBody(meta) {
			const blocks = asArray$8(meta.list);
			if (blocks.length === 0) return null;
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: blocks.map((item, index) => {
					const block = asRecord$17(item);
		...
		function SheetBody(meta) {
			const sheets = asArray$8(asRecord$17(meta.schema).sheets);
			if (sheets.length > 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: sheets.map((item, index) => {
					const table = asRecord$17(item);
					const fields = asArray$8(table.fields).map((field) => asString$11(asRecord$17(field).name)).filter((name) => name !== "");
					return row$1(asString$11(table.name), fields.length === 0 ? "" : `字段：${fields.join(" / ")}`, `t${index}`);
				})
			});
			const table = asRecord$17(meta.table);
			...
			const records = asArray$8(meta.list);
			if (records.length > 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: records.map((item, index) => {
					const record = asRecord$17(item);
					const fields = asRecord$17(record.fieldsValue ?? record.fields);
		...
		function CalendarBody(meta, openPanel) {
			const events = asArray$8(meta.list);
		...
		function ImBody(meta, openPanel) {
			const messages = asArray$8(meta.list);
			if (messages.length > 0 && asString$11(asRecord$17(messages[0]).sendTime) !== "") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: messages.map((item, index) => {
					const message = asRecord$17(item);
					const time = asString$11(message.sendTime).slice(5, 16);
					const content = asString$11(message.content);
					const reply = asString$11(asRecord$17(message.param).replySummary);
			...
			if (groups.length > 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: groups.map((item, index) => {
					...
					const last = asString$11(asRecord$17(group.lastMsg).content);
		...
		function ActionBody(meta, toolName) {
			...
			const recordIds = asArray$8(meta.recordIds);
		...
		*/
		function YzjToolCard({ toolName, block, openPanel }) {
			...
			const meta = asRecord$17(block.meta);
			let body = null;
			if (toolName === "yzj_doc_block_list") body = BlockBody(meta);
			else if (toolName === "yzj_calendar_event_participants") body = listRows(asArray$8(meta.list), ["name"], ["jobTitle", "department"]);
			else if (toolName === "yzj_calendar_room_find") body = listRows(asArray$8(meta.list), ["name", "title"], ["capacity", "floor"]);
		...
		function YzjSessionShell(props) {
			...
			(0, react.useEffect)(() => {
				...
				const tick = async () => {
					const result = await props.homeBinding(props.sessionId);
					if (cancelled || !result.ok) return;
					const binding = asRecord$15(asRecord$15(result.value).binding);
					setRoomKind(binding.yzjKind === "dm" ? "dm" : binding.yzjKind === "group" ? "group" : "");
				};
				tick();
				const timer = window.setInterval(() => {
			...
			const handle = (0, _deepseek_ai_dsh_client_runtime_client.defineStore)({
				...
				actions: {
					...
					appendGroups: (d, groups) => {
						const seen = new Set(d.groups.map((group) => String(asRecord$14(group).groupId)));
						d.groups = [...d.groups, ...groups.filter((group) => !seen.has(String(asRecord$14(group).groupId)))];
					},
					setGroupId: (d, id) => {
						d.groupId = id;
					},
					...
					prependMessages: (d, messages) => {
						const seen = new Set(d.messages.map((message) => String(asRecord$14(message).msgId)));
						d.messages = [...messages.filter((message) => !seen.has(String(asRecord$14(message).msgId))), ...d.messages];
					},
					appendMessages: (d, messages) => {
						const seen = new Set(d.messages.map((message) => String(asRecord$14(message).msgId)));
						d.messages = [...d.messages, ...messages.filter((message) => !seen.has(String(asRecord$14(message).msgId)))];
					},
					setAnchorMsgId: (d, id) => {
						d.anchorMsgId = id;
					},
		...
		*/
		...
		function parseRecentGroups(value) {
			const rec = asRecord$11(value);
			return {
				rooms: asArray$7(rec.list).flatMap((row) => {
		...
		*/
		function MessageBody({ message, onOpenImage, onOpenPdf, inject }) {
			...
			if (msgType === "other" && asString$8(param.title) === "" && asRecord$10(param.interactiveCard).cardJson === void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
			...
			if (msgType === "richText") {
				const desc = asArray$6(param.desc);
				const images = [];
				const bolds = [];
		...
		function openPanelTarget(target, anchorMsgId) {
			...
			if (target.kind === "group") {
				...
				c.inject.fetchMessages(target.groupId, 20).then((result) => {
					if (!result.ok) return;
					const list = asArray$5(asRecord$8(result.value).list);
				...
				c.inject.fetchDocs(target.workspaceId).then((result) => {
					if (result.ok) actions.setDocs(asArray$5(result.value));
				});
			} else {
		...
		*/
		...
		function unreadTotalOf(value) {
			return asArray$4(asRecord$6(value).list).reduce((sum, item) => {
		...
		function loadTab(tab, props) {
			...
			if (tab === "docs") props.fetchWorkspaces().then((result) => {
				...
				props.fetchGroups(20, 1).then((result) => {
					if (result.ok) {
						const groups = asArray$4(asRecord$6(result.value).list);
		...
		function YzjPanel(props) {
			...
			(0, react.useEffect)(() => {
				...
				const poll = () => {
					...
					(anchor === "" ? props.fetchMessages(state.groupId, 20) : props.fetchMessages(state.groupId, 20, {
						...
						if (!result.ok) return;
						const fresh = asArray$4(asRecord$6(result.value).list);
			...
			const runDocSearch = async () => {
				...
				const rows = asArray$4(result.value).length > 0 ? asArray$4(result.value) : asArray$4(asRecord$6(result.value).list);
			...
			const loadDocPreview = (id) => {
				...
				Promise.all([props.fetchDoc(id), props.fetchDocBlocks(id)]).then(([infoResult, blocksResult]) => {
					...
					if (blocksResult.ok) {
						...
						const blocks = asArray$4(asRecord$6(blocksValue.data).blocks ?? blocksValue.blocks);
						const walk = (node2) => {
			...
			const openGroup = (id) => {
				...
				props.fetchMessages(id, 20).then((result) => {
					...
					if (result.ok) {
						const messages = asArray$4(asRecord$6(result.value).list);
			...
			const loadMoreGroups = () => {
				...
				props.fetchGroups(20, state.groupsPage + 1).then((result) => {
					if (result.ok) {
						props.actions.appendGroups(asArray$4(asRecord$6(result.value).list));
			...
			const loadOlderMessages = () => {
				...
				props.fetchMessages(state.groupId, 20, {
					...
					if (gen !== openGenRef.current) return;
					if (result.ok) {
						const older = asArray$4(asRecord$6(result.value).list);
