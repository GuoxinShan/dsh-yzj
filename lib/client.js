window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SheetBody(meta) {
			const sheets = asArray$10(asRecord$20(meta.schema).sheets);
			if (sheets.length > 0) return (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: sheets.map((item, index) => {
					const table = asRecord$20(item);
					const fields = asArray$10(table.fields).map((field) => asString$14(asRecord$20(field).name)).filter((name) => name !== "");
		...
		function AdvanceBody(meta, toolName, jump) {
			...
			if (toolName === "yzj_advance_get") {
				...
				const entries = asArray$10(meta.entries);
			...
			if (toolName === "yzj_advance_inspect") {
				...
				for (let index = 0; index < Math.min(list.length, 8); index += 1) {
					...
					const next = asArray$10(item.next).map((part) => asString$14(part)).filter((part) => part !== "").join(" / ");
			...
			if (toolName === "yzj_advance_scan") {
				const groups = asArray$10(meta.groups);
				const signals = asArray$10(meta.signals);
