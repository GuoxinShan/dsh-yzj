	...
	factory: (require) => {
		...
		*  已归档 (S10: view-layer hide, recoverable) — neither occupies a lane. */
		function lanesOf(todos) {
			...
			const pick = (status) => live.filter((todo) => laneStatusOf(todo) === status).sort(byDdl);
			return {
				lanes: [
					{
						key: "backlog",
				...
				cancelled: live.filter((todo) => laneStatusOf(todo) === "cancelled"),
		...
		function TodoPane(props) {
			...
			const onToggle = (todo) => {
				...
				props.actions.patchTodo({
					...todo,
					status: laneStatusOf(todo) === "done" ? "in_progress" : "done"
				});
