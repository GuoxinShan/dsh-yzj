window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function DocBody(meta, openPanel, listKind) {
			const list = asArray$11(meta.list);
		...
		function BlockBody(meta) {
			const blocks = asArray$11(meta.list);
		...
		function SheetBody(meta) {
			const sheets = asArray$11(asRecord$20(meta.schema).sheets);
			if (sheets.length > 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: sheets.map((item, index) => {
					const table = asRecord$20(item);
					const fields = asArray$11(table.fields).map((field) => asString$14(asRecord$20(field).name)).filter((name) => name !== "");
			...
			const records = asArray$11(meta.list);
		...
		function CalendarBody(meta, openPanel) {
			const events = asArray$11(meta.list);
		...
		function ImBody(meta, openPanel) {
			const messages = asArray$11(meta.list);
			if (messages.length > 0 && asString$14(asRecord$20(messages[0]).sendTime) !== "") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
		...
		function TodoBody(meta, toolName) {
			...
			if (toolName === "yzj_todo_list") {
				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					children: list.map((item, index) => {
						...
						const tags = asArray$11(todo.tags).filter((tag) => typeof tag === "string");
			...
			const tags = asArray$11(meta.tags).filter((tag) => typeof tag === "string");
			const sub = [asString$14(meta.ddl) === "" ? "" : `DDL ${asString$14(meta.ddl)}`, tags.length === 0 ? "" : tags.map((tag) => `#${tag}`).join(" ")].filter((part) => part !== "").join(" · ");
			if (toolName === "yzj_todo_create") rowsOut.push(row$1(meta.idempotentHit === true ? `已存在：${title}` : `已创建：${title}（待人批准）`, sub, "c"));
			else if (toolName === "yzj_todo_complete") rowsOut.push(row$1(`已完成：${title}`, sub, "d"));
			else if (toolName === "yzj_todo_claim") rowsOut.push(row$1(`已认领：${title}`, "版本 v" + asString$14(meta.version) + " · 干完用 yzj_todo_submit_review 交卷", "c"));
			else if (toolName === "yzj_todo_submit_review") rowsOut.push(row$1(`已交卷：${title}`, asString$14(meta.reviewNote), "u"));
			else if (toolName === "yzj_todo_release_claim") rowsOut.push(row$1(`已释放认领：${title}`, asString$14(meta.note), "u"));
			else {
				const changes = asArray$11(meta.changes).filter((change) => typeof change === "string");
		...
		function AdvanceBody(meta, toolName, jump) {
			...
			if (toolName === "yzj_advance_get") {
				...
				const entries = asArray$11(meta.entries);
			...
			if (toolName === "yzj_advance_inspect") {
				...
				for (let index = 0; index < Math.min(list.length, 8); index += 1) {
					...
					const next = asArray$11(item.next).map((part) => asString$14(part)).filter((part) => part !== "").join(" / ");
			...
			if (toolName === "yzj_advance_scan") {
				const groups = asArray$11(meta.groups);
				const signals = asArray$11(meta.signals);
		...
		function ActionBody(meta, toolName) {
			...
			const recordIds = asArray$11(meta.recordIds);
		...
		*/
		function YzjToolCard({ toolName, block, openPanel }) {
			...
			else if (toolName === "yzj_calendar_event_participants") body = listRows(asArray$11(meta.list), ["name"], ["jobTitle", "department"]);
			else if (toolName === "yzj_calendar_room_find") body = listRows(asArray$11(meta.list), ["name", "title"], ["capacity", "floor"]);
		...
		function HandoffModal(props) {
			...
			(0, react.useEffect)(() => {
				props.fetchGroups(20).then((result) => {
					if (!result.ok) return;
					const rows = asArray$10(asRecord$19(result.value).list).map((item) => {
				...
				props.homeDigest(props.sessionId).then((result) => {
					if (!result.ok) return;
					const list = asArray$10(asRecord$19(result.value).candidates);
					setCandidates(list);
		...
		*/
		...
		function parseRecentGroups(value) {
			...
			return {
				rooms: asArray$9(rec.list).flatMap((row) => {
		...
		*/
		function MessageBody({ message, onOpenImage, onOpenPdf, inject }) {
			...
			if (msgType === "other" && asString$11(param.title) === "" && asRecord$13(param.interactiveCard).cardJson === void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: panel_module_css_default.msgSystem,
				children: content === "" ? "(系统消息)" : emojiText(content)
			});
			if (asString$11(param.sysType) === "withdrawMsg") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: panel_module_css_default.msgSystem,
		...
		function AdvanceFeedPicker(props) {
			...
			(0, react.useEffect)(() => {
				...
				props.advanceState().then((result) => {
					...
					const raw = asArray$7(asRecord$12(result.value).items);
					const rows = [];
		...
		*/
		function openPanelTarget(target, anchorMsgId) {
			...
			if (target.kind === "group") {
				...
				c.inject.fetchMessages(target.groupId, 20).then((result) => {
					if (!result.ok) return;
					const list = asArray$6(asRecord$10(result.value).list);
				...
				c.inject.todoState().then((result) => {
					...
					actions.setTodoState(asArray$6(value.todos), value.ready === true);
				});
			} else {
		...
		const css$4 = ".iwUIxq_body{flex-direction:column;flex:1;gap:8px;min-height:0;padding:10px 12px 12px;display:flex;overflow-y:auto}.iwUIxq_quick{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:10px;align-items:center;gap:8px;padding:6px 8px 6px 10px;transition:border-color .15s;display:flex}.iwUIxq_quick:focus-within{border-color:var(--dsw-static-deepseek-500)}.iwUIxq_quickPlus{color:var(--dsw-alias-label-tertiary);user-select:none;font-size:15px;line-height:1}.iwUIxq_quickInput{min-width:0;color:var(--dsw-alias-label-primary);background:0 0;border:none;outline:none;flex:1;font-family:inherit;font-size:13px;line-height:20px}.iwUIxq_quickInput::placeholder{color:var(--dsw-alias-label-caption)}.iwUIxq_quickAdd{background:var(--dsw-alias-button-info-fill);color:var(--dsw-alias-label-primary-foreground);cursor:pointer;border:none;border-radius:7px;flex-shrink:0;padding:7px 10px;font-size:12px;line-height:1}.iwUIxq_quickAdd:hover{background:var(--dsw-alias-button-info-hover)}.iwUIxq_quickAddDisabled{opacity:.45;cursor:default}.iwUIxq_quickHint{color:var(--dsw-alias-label-tertiary);margin:-2px 2px 0 26px;font-size:12px;line-height:16px}.iwUIxq_quickHint strong{color:var(--dsw-alias-label-secondary);font-weight:600}.iwUIxq_tagRail{flex-wrap:wrap;align-items:center;gap:6px;display:flex}.iwUIxq_tagChip{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-secondary);cursor:pointer;white-space:nowrap;border-radius:999px;padding:5px 9px;font-size:12px;line-height:1;transition:border-color .15s,color .15s,background .15s}.iwUIxq_tagChip:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.iwUIxq_tagChipActive{border-color:var(--dsw-static-deepseek-500);background:var(--dsw-static-deepseek-100);color:var(--dsw-static-deepseek-600)}.iwUIxq_lanes{flex:1;align-items:flex-start;gap:10px;min-height:0;display:flex;overflow-x:auto}.iwUIxq_lane{background:var(--dsw-alias-bg-layer-1);border-radius:10px;flex-direction:column;flex:1 1 0;gap:6px;min-width:176px;padding:8px;display:flex}.iwUIxq_laneHint{color:var(--dsw-alias-label-caption);padding:0 2px;font-size:11px;line-height:15px}.iwUIxq_laneEmpty{color:var(--dsw-alias-label-caption);text-align:center;padding:10px 0;font-size:11px}.iwUIxq_bucketHead{align-items:center;gap:6px;padding:0 2px;font-size:12px;font-weight:600;display:flex}.iwUIxq_bucketCount{background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-tertiary);border-radius:999px;padding:1px 7px;font-size:11px;font-weight:500}.iwUIxq_tone-danger{color:var(--dsw-static-red-400)}.iwUIxq_tone-danger .iwUIxq_bucketCount{background:var(--dsw-static-red-100,#ffe4e199);color:var(--dsw-static-red-400)}.iwUIxq_tone-warn{color:#b25e00}.iwUIxq_tone-info{color:var(--dsw-static-deepseek-600)}.iwUIxq_tone-muted{color:var(--dsw-alias-label-secondary)}.iwUIxq_tone-done{color:var(--dsw-alias-label-tertiary)}.iwUIxq_card{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);border-radius:10px;flex-direction:column;gap:6px;padding:8px 9px;transition:border-color .15s;display:flex}.iwUIxq_card:hover{border-color:var(--dsw-static-deepseek-500)}.iwUIxq_cardOverdue{border-color:var(--dsw-static-red-100,#ffe4e1d9)}.iwUIxq_cardDone{opacity:.62}.iwUIxq_cardDone .iwUIxq_rowTitle{text-decoration:line-through;text-decoration-color:var(--dsw-alias-label-caption)}.iwUIxq_cardDesc{color:var(--dsw-alias-label-secondary);-webkit-line-clamp:3;white-space:pre-wrap;word-break:break-word;-webkit-box-orient:vertical;font-size:12px;line-height:16px;display:-webkit-box;overflow:hidden}.iwUIxq_claimBadge{background:var(--dsw-static-deepseek-100);color:var(--dsw-static-deepseek-600);border-radius:5px;align-self:flex-start;padding:3px 7px;font-size:11px;line-height:1}.iwUIxq_reviewBox{border-left:2px solid var(--dsw-static-deepseek-500);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);white-space:pre-wrap;word-break:break-word;border-radius:0 7px 7px 0;padding:6px 8px;font-size:12px;line-height:17px}.iwUIxq_cardVerbs{flex-wrap:wrap;align-items:center;gap:6px;display:flex}.iwUIxq_verb{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-secondary);cursor:pointer;border-radius:7px;padding:5px 9px;font-size:11.5px;line-height:1;transition:border-color .15s,color .15s,background .15s}.iwUIxq_verb:hover{border-color:var(--dsw-static-deepseek-500);color:var(--dsw-static-deepseek-600)}.iwUIxq_verb:disabled{opacity:.55;cursor:wait}.iwUIxq_verbPrimary{border-color:var(--dsw-alias-button-info-fill);background:var(--dsw-alias-button-info-fill);color:var(--dsw-alias-label-primary-foreground)}.iwUIxq_verbPrimary:hover{background:var(--dsw-alias-button-info-hover);border-color:var(--dsw-alias-button-info-hover);color:var(--dsw-alias-label-primary-foreground)}.iwUIxq_verbDanger{color:var(--dsw-alias-label-caption)}.iwUIxq_verbDanger:hover{border-color:var(--dsw-static-red-400);color:var(--dsw-static-red-400)}.iwUIxq_noteForm{align-items:center;gap:6px;display:flex}.iwUIxq_noteInput{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);min-width:0;color:var(--dsw-alias-label-primary);border-radius:7px;outline:none;flex:1;padding:5px 8px;font-family:inherit;font-size:12px;line-height:18px}.iwUIxq_noteInput:focus{border-color:var(--dsw-static-deepseek-500)}.iwUIxq_editForm{border-top:1px dashed var(--dsw-alias-border-l2);flex-direction:column;gap:6px;padding-top:7px;display:flex}.iwUIxq_editArea{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);resize:vertical;border-radius:7px;outline:none;padding:6px 8px;font-family:inherit;font-size:12px;line-height:17px}.iwUIxq_editArea:focus{border-color:var(--dsw-static-deepseek-500)}.iwUIxq_editRow{gap:6px;display:flex}.iwUIxq_closedZone{border-top:1px dashed var(--dsw-alias-border-l2);flex-direction:column;gap:6px;padding-top:6px;display:flex}.iwUIxq_closedToggle{color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;align-self:flex-start;padding:3px 2px;font-size:12px}.iwUIxq_closedToggle:hover{color:var(--dsw-alias-label-primary)}.iwUIxq_row{background:var(--dsw-alias-bg-layer-1);border:1px solid #0000;border-radius:10px;align-items:flex-start;gap:9px;padding:7px 9px;transition:border-color .15s,background .15s;display:flex}.iwUIxq_row:hover{background:var(--dsw-alias-interactive-bg-hover)}.iwUIxq_rowMain{text-align:left;cursor:pointer;background:0 0;border:none;flex-direction:column;flex:1;gap:3px;min-width:0;padding:0;font-family:inherit;display:flex}.iwUIxq_rowTitle{color:var(--dsw-alias-label-primary);word-break:break-word;font-size:13px;line-height:18px}.iwUIxq_rowMeta{flex-wrap:wrap;align-items:center;gap:5px;display:flex}.iwUIxq_chip{background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);border-radius:5px;padding:3px 6px;font-size:11px;line-height:1}.iwUIxq_chipDanger{background:var(--dsw-static-red-100,#ffe4e199);color:var(--dsw-static-red-400)}.iwUIxq_chipWarn{color:#8a5300;background:#fff0d6d9}.iwUIxq_chipTag{background:var(--dsw-static-deepseek-100);color:var(--dsw-static-deepseek-600);cursor:pointer;border-radius:5px;padding:3px 6px;font-size:11px;line-height:1}.iwUIxq_chipTag:hover{background:var(--dsw-static-deepseek-500);color:var(--dsw-alias-label-primary-foreground)}.iwUIxq_chipMuted{color:var(--dsw-alias-label-tertiary);font-size:11px;line-height:1}.iwUIxq_detail{border-left:2px solid var(--dsw-alias-border-l3,var(--dsw-alias-border-l2));flex-direction:column;gap:3px;padding:2px 0 2px 10px;display:flex}.iwUIxq_detailLine{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:16px}.iwUIxq_detailLog{background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-tertiary);border-radius:7px;flex-direction:column;gap:2px;padding:6px 8px;font-size:11px;line-height:16px;display:flex}.iwUIxq_detailHint{color:var(--dsw-alias-label-caption);font-size:11px}.iwUIxq_empty{color:var(--dsw-alias-label-tertiary);flex-direction:column;flex:1;justify-content:center;align-items:center;gap:8px;padding:36px 0;font-size:13px;display:flex}.iwUIxq_emptyIcon{opacity:.75;font-size:26px}.iwUIxq_hero{text-align:center;flex-direction:column;flex:1;justify-content:center;align-items:center;gap:10px;padding:40px 24px;display:flex}.iwUIxq_heroIcon{background:var(--dsw-static-deepseek-100);width:44px;height:44px;color:var(--dsw-static-deepseek-600);border-radius:50%;justify-content:center;align-items:center;font-size:20px;font-weight:700;display:flex}.iwUIxq_heroTitle{color:var(--dsw-alias-label-primary);font-size:15px;font-weight:600}.iwUIxq_heroText{color:var(--dsw-alias-label-secondary);max-width:320px;font-size:12px;line-height:18px}.iwUIxq_heroButton{background:var(--dsw-alias-button-info-fill);color:var(--dsw-alias-label-primary-foreground);cursor:pointer;border:none;border-radius:9px;margin-top:4px;padding:9px 22px;font-size:13px}.iwUIxq_heroButton:hover{background:var(--dsw-alias-button-info-hover)}.iwUIxq_foot{color:var(--dsw-alias-label-caption);text-align:center;padding-top:2px;font-size:11px;line-height:15px}.iwUIxq_notice{background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);max-width:92%;color:var(--dsw-alias-label-secondary);border-radius:8px;align-self:center;padding:7px 12px;font-size:12px;position:sticky;bottom:8px;box-shadow:0 4px 16px #0000002e}";
		...
		var todo_pane_module_css_default = {
			...
			"card": "iwUIxq_card",
			"reviewBox": "iwUIxq_reviewBox",
			"heroTitle": "iwUIxq_heroTitle",
			"editForm": "iwUIxq_editForm",
			"lanes": "iwUIxq_lanes",
		...
		* 已归档 (S10: view-layer hide, recoverable) — neither occupies a lane. */
		function lanesOf(todos) {
			...
			const pick = (status) => live.filter((todo) => asString$7(todo.status) === status).sort(byDdl);
			return {
				lanes: [
					{
						key: "backlog",
					...
					},
					{
						...
						tone: "done",
						hint: "",
						todos: pick("done").reverse().slice(0, 10)
					}
				],
				cancelled: live.filter((todo) => asString$7(todo.status) === "cancelled"),
				archived: todos.filter((todo) => todo.archived === true)
			};
		}
		function TodoPane(props) {
			...
			const [noteFor, setNoteFor] = (0, react.useState)(null);
			const [noteDraft, setNoteDraft] = (0, react.useState)("");
			/** 行内编辑（S7）：标题 + 描述（提示词本体）+ DDL + 负责人。 */
			const [editFor, setEditFor] = (0, react.useState)("");
			...
			const [showCancelled, setShowCancelled] = (0, react.useState)(false);
			...
			const tagCounts = (0, react.useMemo)(() => {
				...
				for (const todo of todos) {
					...
					for (const tag of asTags(todo.tags)) counts.set(tag, (counts.get(tag) ?? 0) + 1);
				}
				return [...counts.entries()].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));
			}, [todos]);
			const visible = props.tagFilter === "" ? todos : todos.filter((todo) => asTags(todo.tags).includes(props.tagFilter));
			const { lanes, cancelled, archived } = (0, react.useMemo)(() => lanesOf(visible), [visible]);
			const openCount = todos.filter((todo) => todo.archived !== true && asString$7(todo.status) !== "done" && asString$7(todo.status) !== "cancelled").length;
			const flash = (message) => {
				setNotice(message);
			...
			const applyState = (value) => {
				...
				props.actions.setTodoState(Array.isArray(record.todos) ? record.todos : [], record.ready === true);
			};
			const onEnsure = () => {
				setEnsuring(true);
			...
			const runVerb = (verb, todoId, note) => {
				setBusyId(todoId);
				(verb === "approve" ? props.approveTodo(todoId, note) : verb === "accept" ? props.acceptTodo(todoId, note) : verb === "return" ? props.returnTodo(todoId, note) : verb === "cancel" ? props.cancelTodo(todoId, note) : verb === "archive" ? props.archiveTodo(todoId, true) : verb === "unarchive" ? props.archiveTodo(todoId, false) : props.reopenTodo(todoId)).then((result) => {
					setBusyId("");
					setNoteFor(null);
					setNoteDraft("");
					if (result.ok) {
			...
			const openEdit = (todo) => {
				setEditFor(asString$7(todo.todoId));
				setEditDraft({
					title: asString$7(todo.title),
					description: asString$7(todo.description),
					ddl: asString$7(todo.ddl),
					assignee: asString$7(todo.assignee)
				});
			};
			/** 「让 agent 做」（期②）：host 直建 yzj-todo-* 会话注入任务卡，成功后聚焦。 */
			const onDispatch = (todoId) => {
				setBusyId(todoId);
				props.dispatchTodo(todoId).then((result) => {
					setBusyId("");
					if (result.ok) {
						const sessionId = asString$7(asRecord$9(result.value).sessionId);
			...
			const saveEdit = (todoId) => {
				...
				props.editTodo(todoId, {
					...
					if (result.ok) {
						...
						props.actions.patchTodo(result.value);
						refresh();
					} else flash(`保存失败：${result.error.message}`);
				});
			};
			...
			const renderCard = (todo) => {
				...
				const ddl = asString$7(todo.ddl);
				const terminal = status === "done" || status === "cancelled";
				const overdue = !terminal && ddl !== "" && ddl < todayStr();
				const dueToday = !terminal && ddl === todayStr();
				const description = asString$7(todo.description);
				const claimedBy = asString$7(todo.claimedBy);
				const reviewNote = asString$7(todo.reviewNote);
				const isArchived = todo.archived === true;
				const busy = busyId === todoId;
				return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: `${todo_pane_module_css_default.card}${status === "done" ? ` ${todo_pane_module_css_default.cardDone}` : ""}${status === "cancelled" ? ` ${todo_pane_module_css_default.cardDone}` : ""}${overdue ? ` ${todo_pane_module_css_default.cardOverdue}` : ""}`,
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								children: [
									ddl !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
										...
										children: [overdue ? "逾期 " : dueToday ? "今天 " : "", ddl]
									}),
									...
									asString$7(todo.priority) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: todo_pane_module_css_default.chipMuted,
										children: asString$7(todo.priority)
									}),
									asString$7(todo.assignee) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
										className: todo_pane_module_css_default.chipMuted,
										children: ["@", asString$7(todo.assignee)]
									})
								]
							})]
						}),
						description !== "" && !isArchived && status !== "done" && status !== "cancelled" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: todo_pane_module_css_default.cardDesc,
							title: description,
							children: description
						}),
						status === "in_progress" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: todo_pane_module_css_default.claimBadge,
							children: ["agent 认领中", claimedBy === "" ? "" : ` · ${claimedBy.slice(0, 16)}`]
						}),
						status === "in_review" && reviewNote !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: todo_pane_module_css_default.reviewBox,
							children: reviewNote
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: todo_pane_module_css_default.cardVerbs,
							children: [
								isArchived && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: todo_pane_module_css_default.verb,
									"data-testid": `yzj-todo-unarchive-${todoId}`,
									disabled: busy,
									onClick: () => {
										runVerb("unarchive", todoId);
									},
									children: "恢复"
								}),
								!isArchived && status === "backlog" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: `${todo_pane_module_css_default.verb} ${todo_pane_module_css_default.verbPrimary}`,
										"data-testid": `yzj-todo-approve-${todoId}`,
										disabled: busy,
										onClick: () => {
											runVerb("approve", todoId);
										},
										children: "批准"
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: todo_pane_module_css_default.verb,
										"data-testid": `yzj-todo-edit-${todoId}`,
										disabled: busy,
										onClick: () => {
											openEdit(todo);
										},
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: `${todo_pane_module_css_default.verb} ${todo_pane_module_css_default.verbDanger}`,
										"data-testid": `yzj-todo-cancel-${todoId}`,
										disabled: busy,
										onClick: () => {
											runVerb("cancel", todoId);
										},
										children: "中止"
									})
								] }),
								!isArchived && status === "todo" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: `${todo_pane_module_css_default.verb} ${todo_pane_module_css_default.verbPrimary}`,
										"data-testid": `yzj-todo-dispatch-${todoId}`,
										disabled: busy,
										title: "开一个 agent 会话认领并执行这条待办（期②）",
										onClick: () => {
											onDispatch(todoId);
										},
										children: "让 agent 做"
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: todo_pane_module_css_default.verb,
										"data-testid": `yzj-todo-done-${todoId}`,
										disabled: busy,
										title: "人直写完成的快路径（不经验收）",
										onClick: () => {
											onToggle(todo);
										},
										children: "✓ 完成"
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: todo_pane_module_css_default.verb,
										"data-testid": `yzj-todo-return-${todoId}`,
										disabled: busy,
										onClick: () => {
											setNoteFor({
												todoId,
								...
								!isArchived && status === "in_progress" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: todo_pane_module_css_default.verb,
										"data-testid": `yzj-todo-done-${todoId}`,
								...
								!isArchived && status === "in_review" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: `${todo_pane_module_css_default.verb} ${todo_pane_module_css_default.verbPrimary}`,
									"data-testid": `yzj-todo-accept-${todoId}`,
									disabled: busy,
									onClick: () => {
										setNoteFor({
											todoId,
								...
								!isArchived && status === "done" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: todo_pane_module_css_default.verb,
									"data-testid": `yzj-todo-reopen-${todoId}`,
									disabled: busy,
									onClick: () => {
										onToggle(todo);
									},
									...
									type: "button",
									className: todo_pane_module_css_default.verb,
									"data-testid": `yzj-todo-archive-${todoId}`,
									disabled: busy,
									title: "收进已归档折叠区（可恢复）",
									onClick: () => {
										runVerb("archive", todoId);
									},
									children: "归档"
								})] }),
								!isArchived && status === "cancelled" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: todo_pane_module_css_default.verb,
									"data-testid": `yzj-todo-reopen-${todoId}`,
									disabled: busy,
									onClick: () => {
										runVerb("reopen", todoId);
									},
									children: "重开"
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
						...
						noteFor !== null && noteFor.todoId === todoId && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: todo_pane_module_css_default.noteForm,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
									className: todo_pane_module_css_default.noteInput,
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: `${todo_pane_module_css_default.verb} ${todo_pane_module_css_default.verbPrimary}`,
									"data-testid": `yzj-todo-note-confirm-${todoId}`,
									disabled: busy,
									onClick: () => {
										runVerb(noteFor.verb, todoId, noteDraft);
									},
									children: noteFor.verb === "accept" ? "确认验收" : "确认打回"
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: todo_pane_module_css_default.verb,
						...
						editFor === todoId && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: todo_pane_module_css_default.editForm,
							"data-testid": `yzj-todo-edit-form-${todoId}`,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
									className: todo_pane_module_css_default.noteInput,
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("textarea", {
									className: todo_pane_module_css_default.editArea,
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									className: todo_pane_module_css_default.editRow,
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
										className: todo_pane_module_css_default.noteInput,
										value: editDraft.ddl,
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: `${todo_pane_module_css_default.verb} ${todo_pane_module_css_default.verbPrimary}`,
										"data-testid": `yzj-todo-edit-save-${todoId}`,
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										status,
										ddl === "" ? "" : ` · DDL ${ddl}`,
										asString$7(todo.claimedBy) === "" ? "" : ` · 认领会话 ${asString$7(todo.claimedBy)}`
									]
								}),
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					visible.length === 0 && !props.loading ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						className: todo_pane_module_css_default.lanes,
						children: lanes.map((lane) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
							className: todo_pane_module_css_default.lane,
							"aria-label": lane.label,
							"data-testid": `yzj-todo-lane-${lane.key}`,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
									className: `${todo_pane_module_css_default.bucketHead} ${todo_pane_module_css_default[`tone-${lane.tone}`]}`,
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: lane.label }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: todo_pane_module_css_default.bucketCount,
										children: lane.todos.length
									})]
								}),
								lane.hint !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: todo_pane_module_css_default.laneHint,
									children: lane.hint
								}),
								lane.todos.map(renderCard),
								lane.todos.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: todo_pane_module_css_default.laneEmpty,
									children: "（空）"
								})
							]
						}, lane.key))
					}),
					cancelled.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: todo_pane_module_css_default.closedZone,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
							type: "button",
							className: todo_pane_module_css_default.closedToggle,
					...
					archived.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: todo_pane_module_css_default.closedZone,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
							type: "button",
							className: todo_pane_module_css_default.closedToggle,
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("footer", {
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "泳道待办：你管批准与验收，agent 认领执行；卡片可直接编辑任务描述——那就是 agent 认领后执行的那段提示词" })
					}),
		...
		function loadTab(tab, props) {
			...
			if (tab === "docs") props.fetchWorkspaces().then((result) => {
				...
				if (result.ok) {
					...
					props.actions.setTodoState(asArray$5(value.todos), value.ready === true);
					props.actions.setLoading(false);
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									...
									children: docResults !== null ? docSearching ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										className: panel_module_css_default.empty,
										children: "搜索中…"
									}) : docResults.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: panel_module_css_default.empty,
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(YzjCloudIcon, { size: 28 }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "没有命中文档" })]
									...
									}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [state.workspaces.length === 0 && !state.loading && state.error === "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: panel_module_css_default.empty,
										...
										return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [personal.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
											className: panel_module_css_default.paneGroupLabel,
											"data-testid": "yzj-panel-ws-group-personal",
											children: "个人"
										}), personal.map(renderWs)] }), enterprise.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
											className: panel_module_css_default.paneGroupLabel,
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("i", { className: `${advance_pane_module_css_default.dot} ${advance_pane_module_css_default[`dot_${tone}`]}` }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: asString$4(item.title) === "" ? "(无标题)" : asString$4(item.title) })]
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: asString$4(item.latest) === "" ? STAGE_LABEL[asString$4(item.stage)] ?? asString$4(item.stage) : asString$4(item.latest) })]
					}, id);
				})]
			});
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("aside", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: "我的推进" }),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									...
									dreamLine !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: advance_pane_module_css_default.dreamLine,
										"data-testid": "yzj-advance-dream-status",
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: dreamLine }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
											className: advance_pane_module_css_default.dreamActions,
											children: [dreamEntries.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
												type: "button",
												className: advance_pane_module_css_default.patrolBtn,
							...
							queues.closed.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
									...
									return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
											className: advance_pane_module_css_default.queueTitle,
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("i", { className: `${advance_pane_module_css_default.dot} ${advance_pane_module_css_default[`dot_${dotToneOf(asString$4(item.stage))}`]}` }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: asString$4(item.title) === "" ? "(无标题)" : asString$4(item.title) })]
										}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", { children: [STAGE_LABEL[asString$4(item.stage)] ?? asString$4(item.stage), asString$4(item.latest) === "" ? "" : ` · ${asString$4(item.latest)}`] })]
									}, id);
								})]
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("main", {
						...
						children: [board.loading || detailLoading ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
								...
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: asString$4(detail.item.advanceId) }),
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h1", { children: asString$4(detail.item.title) }),
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: advance_pane_module_css_default.meta,
										children: [
											asString$4(detail.item.assignee) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: ["结果承担者：", asString$4(detail.item.assignee)] }),
											asString$4(detail.item.targetDate) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: ["目标日期：", asString$4(detail.item.targetDate)] }),
							...
							metrics.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
								className: advance_pane_module_css_default.section,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									className: advance_pane_module_css_default.sectionHead,
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", { children: "成功指标" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: "这几项达标 = 推进达到目标" })]
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									...
									children: metrics.map((metric, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: asString$4(metric.name) }),
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: asString$4(metric.current) === "" ? "—" : asString$4(metric.current) }),
											asString$4(metric.target) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("small", { children: ["目标 ", asString$4(metric.target)] })
										]
									}, `m${index}`))
								})]
							}),
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
											...
											children: [
												...
												asString$4(detail.item.background) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
											...
											children: [
												...
												stage === "decision-needed" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
													...
													children: [latestDecision !== void 0 && decisionParsed !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
														/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", { children: asString$4(latestDecision.summary) }),
														decisionParsed.rest !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: decisionParsed.rest }),
														decisionParsed.actions.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
															...
															children: decisionParsed.actions.map((action, actionIndex) => {
																...
																return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
																	...
																	}), action.kind === "im" && !done && imDraft?.key === key && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
														...
														decisionParsed.mergedFrom !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
													...
													] }) : latestDriver === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
														className: advance_pane_module_css_default.quiet,
														children: "等待你处理。"
													}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
														/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", { children: asString$4(latestDriver.summary) }),
														asString$4(latestDriver.detail) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: asString$4(latestDriver.detail) }),
														/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
															className: advance_pane_module_css_default.quiet,
												...
												stage === "running" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
													className: advance_pane_module_css_default.quiet,
													children: "AI 正在跟进，当前不需要你处理；有目标变化或材料不足时会再提醒。"
												}),
												(stage === "completed" || stage === "cancelled") && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
													className: advance_pane_module_css_default.decision,
												...
												stage !== "completed" && stage !== "cancelled" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
													className: advance_pane_module_css_default.verbs,
													children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
														type: "button",
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
											...
											children: [
												/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
													className: advance_pane_module_css_default.sectionHead,
													children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", { children: "推进演进" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: "每条事元可溯源到原始信息" })]
												}),
													...
													children: [...detail.entries].reverse().map((entry, index) => {
														...
														return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
															...
															children: [
																...
																/* @__PURE__ */ (0, react_jsx_runtime.jsx)("i", { className: `${advance_pane_module_css_default.mark} ${advance_pane_module_css_default[`mark_${asString$4(entry.tone) || "blue"}`]}` }),
																/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
																	...
																	children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
																		...
																		children: [
																			asString$4(entry.changeType) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
																		...
																		asString$4(entry.detail) !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
																		...
																		refList.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
																			...
																			children: refList.map((raw) => {
																				...
																				if (href !== null) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("a", {
																				...
																				if (kind === "msg" && msgAnchorOf(id) !== null) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
																					type: "button",
																					className: advance_pane_module_css_default.refChip,
																					title: `打开来源群消息 ${raw}`,
																					"data-testid": `yzj-advance-ref-${id}`,
																				...
																				if (domain !== null) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
																					type: "button",
																					className: advance_pane_module_css_default.refChip,
																					title: raw,
																					"data-testid": `yzj-advance-ref-${id}`,
																		...
																		/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
																			...
																			children: [(() => {
																				...
																				return (origin !== "" || asString$4(entry.actor) === "user") && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: asString$4(entry.actor) === "user" ? `${origin === "" ? "" : `${origin} · `}你的判断` : `记录自 ${origin}` });
																			})(), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
																				type: "button",
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
										...
										children: [
											/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
												className: advance_pane_module_css_default.sectionHead,
												children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", { children: "上下文来源" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
													type: "button",
													className: advance_pane_module_css_default.linkBtn,
											...
											detail.contextSources.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
												className: advance_pane_module_css_default.quiet,
												...
												children: detail.contextSources.map((source, index) => {
													...
													return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
														...
														children: [
															...
															/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: asString$4(source.label) === "" ? token : asString$4(source.label) }),
															/* @__PURE__ */ (0, react_jsx_runtime.jsx)("em", { children: asString$4(source.addedBy) === "user" ? "你关联" : "AI 关联" }),
															/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
																type: "button",
																"aria-label": "解除关联",
											...
											pendingRecs.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
												className: advance_pane_module_css_default.subSources,
												...
												children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
													...
													return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
														...
														children: [
															...
															/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: rec.label }),
															/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
																type: "button",
																className: advance_pane_module_css_default.linkBtn,
					...
					dreamPoolOpen && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: advance_pane_module_css_default.mask,
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
							...
							children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [dreamEntries.map((entry) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: entry.content })]
									}, entry.id)), dreamEntries.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
										className: advance_pane_module_css_default.sideNote,
										children: "池是空的。"
									})]
								})
					...
					sourceModalOpen && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: advance_pane_module_css_default.mask,
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
									className: advance_pane_module_css_default.modalHead,
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", { children: "关联来源" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										"aria-label": "关闭",
										onClick: () => {
											setSourceModalOpen(false);
								...
								groupOptions.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								...
								dirOptions.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
									className: advance_pane_module_css_default.subGroupLabel,
									children: "知识库（整库订阅）"
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: advance_pane_module_css_default.subGroupList,
									"data-testid": "yzj-advance-source-dirs",
									children: dirOptions.map((dir) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
		...
		*/
		...
		function createYzjPanelInject(connection) {
			...
			return {
				...
				approveTodo: (todoId, note) => call("todo-approve", {
					todoId,
					...note === void 0 || note === "" ? {} : { note }
				}),
				acceptTodo: (todoId, note) => call("todo-accept", {
					todoId,
					...note === void 0 || note === "" ? {} : { note }
				}),
				returnTodo: (todoId, note) => call("todo-return", {
					todoId,
					...note === void 0 || note === "" ? {} : { note }
				}),
				cancelTodo: (todoId, note) => call("todo-cancel", {
					todoId,
					...note === void 0 || note === "" ? {} : { note }
				}),
				reopenTodo: (todoId) => call("todo-reopen", { todoId }),
				archiveTodo: (todoId, archived) => call("todo-archive", {
					todoId,
					archived
				}),
				...
				editTodo: (todoId, patch) => call("todo-edit", {
					...
					...patch.ddl === void 0 ? {} : { ddl: patch.ddl },
		...
		function ArgBody({ record, names }) {
			...
			switch (record.domain) {
				...
				case "todo": {
					...
					if (record.toolName === "yzj_todo_update" || record.toolName === "yzj_todo_complete") push("操作", record.toolName === "yzj_todo_complete" ? "标记完成（快路径）" : "更新字段", "op");
