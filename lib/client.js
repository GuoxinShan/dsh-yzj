window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function DocBody(meta, openPanel, listKind) {
			const list = asArray$6(meta.list);
			...
			const record = asRecord$18(meta.record);
		...
		function BlockBody(meta) {
			const blocks = asArray$6(meta.list);
			if (blocks.length === 0) return null;
			return (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: blocks.map((item, index) => {
					const block = asRecord$18(item);
		...
		function SheetBody(meta) {
			const sheets = asArray$6(asRecord$18(meta.schema).sheets);
			if (sheets.length > 0) return (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: sheets.map((item, index) => {
					const table = asRecord$18(item);
					const fields = asArray$6(table.fields).map((field) => asString$11(asRecord$18(field).name)).filter((name) => name !== "");
					return row$1(asString$11(table.name), fields.length === 0 ? "" : `字段：${fields.join(" / ")}`, `t${index}`);
				})
			});
			const table = asRecord$18(meta.table);
			...
			const records = asArray$6(meta.list);
			if (records.length > 0) return (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: records.map((item, index) => {
					const record = asRecord$18(item);
					const fields = asRecord$18(record.fieldsValue ?? record.fields);
		...
		function CalendarBody(meta, openPanel) {
			const events = asArray$6(meta.list);
		...
		function ImBody(meta, openPanel) {
			const messages = asArray$6(meta.list);
			if (messages.length > 0 && asString$11(asRecord$18(messages[0]).sendTime) !== "") return (0, react_jsx_runtime.jsx)("div", {
				className: cards_module_css_default.rows,
				children: messages.map((item, index) => {
					const message = asRecord$18(item);
					const time = asString$11(message.sendTime).slice(5, 16);
					const content = asString$11(message.content);
					const reply = asString$11(asRecord$18(message.param).replySummary);
			...
			if (groups.length > 0) return (0, react_jsx_runtime.jsx)("div", {
				...
				children: groups.map((item, index) => {
					...
					const last = asString$11(asRecord$18(group.lastMsg).content);
		...
		function ActionBody(meta, toolName) {
			...
			const recordIds = asArray$6(meta.recordIds);
		...
		*/
		function YzjToolCard({ toolName, block, openPanel }) {
			...
			const meta = asRecord$18(block.meta);
			let body = null;
			if (toolName === "yzj_doc_block_list") body = BlockBody(meta);
			else if (toolName === "yzj_calendar_event_participants") body = listRows(asArray$6(meta.list), ["name"], ["jobTitle", "department"]);
			else if (toolName === "yzj_calendar_room_find") body = listRows(asArray$6(meta.list), ["name", "title"], ["capacity", "floor"]);
		...
		function YzjHomeChrome(props) {
			...
			(0, react.useEffect)(() => {
				...
				const tick = async () => {
					...
					const raw = asRecord$17(result.value);
					const binding = asRecord$17(raw.binding);
					...
					const topic = asRecord$17(raw.topic);
			...
			const sendToGroup = async () => {
				...
				if (draft === "") return;
				if (!(await props.homeSend(props.sessionId, draft)).ok) return;
				props.clearDraft();
			};
		...
		*/
			...
			const handle = defineStore({
				...
				actions: {
					...
					appendGroups: (d, groups) => {
						const seen = new Set(d.groups.map((group) => String(asRecord$15(group).groupId)));
						d.groups = [...d.groups, ...groups.filter((group) => !seen.has(String(asRecord$15(group).groupId)))];
					},
					setGroupId: (d, id) => {
						d.groupId = id;
					},
					...
					prependMessages: (d, messages) => {
						const seen = new Set(d.messages.map((message) => String(asRecord$15(message).msgId)));
						d.messages = [...messages.filter((message) => !seen.has(String(asRecord$15(message).msgId))), ...d.messages];
					},
					appendMessages: (d, messages) => {
						const seen = new Set(d.messages.map((message) => String(asRecord$15(message).msgId)));
						d.messages = [...d.messages, ...messages.filter((message) => !seen.has(String(asRecord$15(message).msgId)))];
					},
					setAnchorMsgId: (d, id) => {
						d.anchorMsgId = id;
					},
		...
		//#endregion
		//#region lib/types/cli-payload.js
		/**
		...
		*/
		...
		function rowsOf(json) {
			...
			const record = asRecord$13(peeled);
			if (Array.isArray(record.list)) return record.list;
			if (Array.isArray(record.data)) return record.data;
			if (typeof record.data === "object" && record.data !== null) {
				const inner = asRecord$13(record.data);
				if (Array.isArray(inner.list)) return inner.list;
		...
		function parseContactUser(json) {
			const user = asRecord$13(rowsOf(json)[0]);
			return {
		...
		*/
		function YzjLoginBanner(props) {
			...
			const probe = async (afterLogin) => {
				...
				const rec = asRecord$12(result.value);
				if (rec.loggedIn === true) {
					setName(asString$10(rec.name) || asString$10(rec.openId) || "已登录");
					setHint("");
					setPhase("in");
					if (afterLogin) props.onLoggedIn?.();
					return;
				}
				const reason = asString$10(rec.reason);
		...
		*/
		...
		function parseNavRooms(value) {
			return asArray$5(asRecord$11(value).rooms).flatMap((row) => {
				const rec = asRecord$11(row);
				const sessionId = asString$9(rec.sessionId);
				const groupId = asString$9(rec.groupId);
				if (sessionId === "" || groupId === "") return [];
				const topics = asArray$5(rec.topics).flatMap((item) => {
					const topic = asRecord$11(item);
		...
		function parseRecentGroups(value) {
			const outer = asRecord$11(value);
			const rec = Array.isArray(outer.list) ? outer : Array.isArray(asRecord$11(outer.data).list) ? asRecord$11(outer.data) : outer;
			return {
				rooms: asArray$5(rec.list).flatMap((row) => {
					const item = asRecord$11(row);
					const groupId = asString$9(item.groupId);
					if (groupId === "") return [];
					return [{
						groupId,
						groupName: asString$9(item.groupName) || (kindOf(groupId) === "dm" ? "私聊" : "群聊"),
						lastMsg: asRecord$11(item.lastMsg),
		...
		*/
		...
		var panel_module_css_default = {
			...
			"docSearch": "lIVaIG_docSearch",
			"docSearchInput": "lIVaIG_docSearchInput",
			"drill": "lIVaIG_drill",
			...
			"paneGroupLabel": "lIVaIG_paneGroupLabel",
		...
		*/
		function MessageBody({ message, onOpenImage, onOpenPdf, inject }) {
			...
			if (msgType === "other" && asString$8(param.title) === "" && asRecord$10(param.interactiveCard).cardJson === void 0) return (0, react_jsx_runtime.jsx)("span", {
				className: panel_module_css_default.msgSystem,
				children: content === "" ? "(系统消息)" : emojiText(content)
			});
			if (asString$8(param.sysType) === "withdrawMsg") return (0, react_jsx_runtime.jsx)("span", {
				className: panel_module_css_default.msgSystem,
			...
			if (msgType === "other") {
				const cardJson = asString$8(asRecord$10(param.interactiveCard).cardJson);
				const face = cardJson === "" ? {
					title: "",
					image: "",
					actionTitle: "",
		...
		*/
		function artifactOf(entry) {
			...
			const ext = asString$7(param.ext);
			return {
		...
		function YzjFusedView(props) {
			...
			(0, react.useEffect)(() => {
				...
				const load = async (backfill) => {
					...
					setError("");
					const next = applyFused(asRecord$9(fused.value));
					...
					if (backfill) {
						...
						if (cancelled) return;
						if (stats.ok) {
							if (asRecord$9(stats.value).more === false) setMore(false);
							const again = await props.homeFused(props.sessionId, props.groupId);
							if (!cancelled && again.ok) applyFused(asRecord$9(again.value));
						}
					}
			...
			const loadOlder = async () => {
				...
				if (!result.ok) return;
				const seeded = seedNames(applyFused(asRecord$9(result.value)).items);
				if (Object.keys(seeded).length > 0) setNames((prev) => ({
					...seeded,
					...prev
				}));
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("div", {
						...
						children: (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [emptyPhase ? (0, react_jsx_runtime.jsx)("div", {
		...
		*/
		...
		function eventsOn(events, day) {
			return events.map(asRecord$7).filter((event) => eventDayKey(event) === day);
		}
		...
		*/
		function CalendarPane(props) {
			...
			const renderEventChip = (event, key) => {
				...
				const tone = toneOf(id === "" ? asString$5(event.title) : id);
				...
				return (0, react_jsx_runtime.jsxs)("button", {
					...
					children: [(0, react_jsx_runtime.jsx)("span", {
						...
						children: asString$5(event.title)
					})]
				}, key);
			};
		...
		*/
		...
		function groupNameOf(groups, groupId) {
			...
			return group === void 0 ? "" : asString$4(group.groupName);
		}
		...
		function unreadTotalOf(value) {
			return cliRows(value).reduce((sum, item) => {
		...
		function YzjPanel(props) {
			...
			const storedTab = tab === "docs" || tab === "calendar" || tab === "chat" ? tab : "docs";
			const activeTab = embedded ? props.forceTab ?? (storedTab === "chat" ? "docs" : storedTab) : storedTab;
			...
			const runDocSearch = async () => {
				...
				setDocResults(rows.map(asRecord$6));
			};
			...
			const fetchDocsAt = (workspace, parentId) => {
				...
				props.fetchDocs(workspace, parentId).then((result) => {
					if (result.ok) props.actions.setDocs(cliRows(result.value));
			...
			const loadDocPreview = (id) => {
				...
				Promise.all([props.fetchDoc(id), props.fetchDocBlocks(id)]).then(([infoResult, blocksResult]) => {
					...
					if (blocksResult.ok) {
						...
						const blocks = asArray$3(asRecord$6(blocksValue.data).blocks ?? blocksValue.blocks);
						const walk = (node2) => {
							...
							children: [(0, react_jsx_runtime.jsxs)("div", {
								...
								children: [(0, react_jsx_runtime.jsx)("div", {
									...
									children: docResults !== null ? docSearching ? (0, react_jsx_runtime.jsx)("div", {
										...
										const title = asString$4(node.title) || asString$4(node.fileName) || id;
										const updated = asString$4(node.updateTime).slice(0, 10);
										const kb = asString$4(node.kbName);
										return (0, react_jsx_runtime.jsxs)("button", {
											type: "button",
										...
										const rows = state.workspaces.map(asRecord$6);
		...
		*/
		function YzjRoomShell(props) {
			...
			const domainPane = domain !== "im" && props.panel !== void 0 && props.useStore !== void 0 && props.actions !== void 0 ? (0, react_jsx_runtime.jsx)(YzjDomainWorkbench, {
				domain,
				panel: props.panel,
				useStore: props.useStore,
				actions: props.actions
		...
		*/
		...
		const tagId$1 = "@dsh-yzj/bundle/overlay.module.css";
		...
		*/
		...
		function blockText(node) {
			...
			const childArray = asArray$2(record.childNodes ?? record.children);
			...
			try {
				switch (ref.kind) {
					case "workspace": {
						...
						if (result.ok) {
							const ws = asRecord$3(unwrapCli(result.value));
					...
					case "doc": {
						...
						if (infoResult.ok) {
							const node = asRecord$3(unwrapCli(infoResult.value));
						...
						if (blocksResult.ok) {
							const blocksValue = asRecord$3(unwrapCli(blocksResult.value));
							const excerpt = asArray$2(asRecord$3(blocksValue.data).blocks ?? blocksValue.blocks).slice(0, 10).map(blockText).filter((text) => text !== "").join(" ");
							if (excerpt !== "") {
								lines.push(`内容摘要：${excerpt.length > 500 ? `${excerpt.slice(0, 500)}…` : excerpt}`);
								lines.push("（内容为摘要，完整内容可用 yzj_doc_block_list / yzj_doc_get 获取）");
							}
						}
						if (infoResult.ok && asString$3(asRecord$3(unwrapCli(infoResult.value)).fileSuffix) === "dbt") {
							const sheetResult = await inject.fetchSheet(ref.id);
							if (sheetResult.ok) {
								const sheetValue = asRecord$3(unwrapCli(sheetResult.value));
								const tableLines = asArray$2(sheetValue.sheets ?? asRecord$3(sheetValue.data).sheets).slice(0, 5).map((item) => {
									const table = asRecord$3(item);
									const fields = asArray$2(table.fields).map((field) => asString$3(asRecord$3(field).name)).filter((name) => name !== "");
					...
					case "event": {
						...
						if (result.ok) {
							const event = asRecord$3(unwrapCli(result.value));
					...
					case "contact": {
						...
						if (result.ok) {
							const rec = asRecord$3(unwrapCli(result.value));
							const parts = [
								asString$3(rec.department),
								asString$3(rec.jobTitle),
								asString$3(rec.jobNo) === "" ? "" : `工号 ${asString$3(rec.jobNo)}`
							];
					...
					case "message": {
						...
						if (groupId !== "") {
							...
							if (result.ok) {
								const hit = cliRows(result.value).find((item) => asString$3(asRecord$3(item).msgId) === ref.id);
								if (hit !== void 0) {
		...
		const tagId = "@dsh-yzj/bundle/settings-section.module.css";
		...
		function useResolvedNames(record, inject) {
			...
			(0, react.useEffect)(() => {
				...
				if (record.domain === "im" && groupId !== "" && inject.fetchGroups !== void 0) tasks.push(inject.fetchGroups(20).then((result) => {
					if (!result.ok) return;
					const group = cliRows(result.value).map(asRecord$1).find((g) => asString$1(g.groupId) === groupId);
				...
				if ((record.domain === "doc" || record.domain === "sheet") && docId !== "" && inject.fetchDoc !== void 0) tasks.push(inject.fetchDoc(docId).then((result) => {
					if (!result.ok) return;
					const node = asRecord$1(unwrapCli(result.value));
				...
				if (workspace !== "" && inject.fetchWorkspaces !== void 0) tasks.push(inject.fetchWorkspaces().then((result) => {
					if (!result.ok) return;
					const ws = cliRows(result.value).map(asRecord$1).find((w) => asString$1(w.id) === workspace);
				...
				if (inject.fetchContact !== void 0) for (const raw of asArray$1(args.organizerOpenIds)) {
					...
					tasks.push(inject.fetchContact(openId).then((result) => {
						if (!result.ok) return;
						const name = asString$1(asRecord$1(cliRows(result.value)[0] ?? unwrapCli(result.value)).name);
