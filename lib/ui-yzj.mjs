...
function dreamAskPrompt() {
	return "请做一轮 Dream 抽取:先 yzj_advance_dream_status 读蓄水池 pending 清单,然后逐条与 open 事项比对(yzj_advance_inspect):有价值的按纪律 feed(refs 用池条目的 channel+refId 组装成 im:<groupId>:<msgId> token 抄进去,面板才能定位到具体群消息;进度正常静默挂,命中打扰判据才 stageTo=decision-needed 形成建议卡片),无关的跳过;最后 yzj_advance_dream_mark(ids=[已处理条目 id]) 并给我一句「抽取 N 条/产出 M 条建议」的总结。直接连续调用工具完成,不要询问我。";
}
	...
	return async (endpoint, payload, _signal) => {
		switch (endpoint) {
			...
			case "advance-ref-lookup": {
				...
				const raw = typeof payload === "object" && payload !== null ? payload.tokens : void 0;
