...
function calendarEventsFromJson(json) {
	...
	if (Array.isArray(record.list)) return record.list;
	if (Array.isArray(record.data)) return record.data;
	const nested = asRecord$3(record.data);
...
function mergeCalendarEvents(records) {
	...
	for (const record of records) {
		const id = asString$2(asRecord$3(record).id);
	...
	out.sort((left, right) => {
		return (asNumber(asRecord$3(left).startDate) ?? 0) - (asNumber(asRecord$3(right).startDate) ?? 0);
	});
	return out;
}
...
function filterCalendarEvents(records, start, end) {
	...
	return records.filter((record) => {
		const ms = asNumber(asRecord$3(record).startDate);
...
//#endregion
//#region packages/tool-yzj/src/advance-threads.ts
/**
	...
	return async (endpoint, payload, _signal) => {
		switch (endpoint) {
			...
			case "advance-thread-add": {
				const advance = ctx.get("yzjAdvance");
				if (advance === void 0) return internalError("advance-thread-add: yzjAdvance 服务不可用（tool-yzj 未挂载）");
				const advanceId = stringField(payload, "advanceId");
				const token = stringField(payload, "token");
				if (advanceId === void 0 || token === void 0) return internalError("advance-thread-add endpoint requires advanceId and token payloads");
				try {
					return {
						ok: true,
						value: await advance.threadAdd(advanceId, token, stringField(payload, "label"))
					};
				} catch (error) {
			...
			case "advance-thread-remove": {
				const advance = ctx.get("yzjAdvance");
				if (advance === void 0) return internalError("advance-thread-remove: yzjAdvance 服务不可用（tool-yzj 未挂载）");
				const advanceId = stringField(payload, "advanceId");
				const token = stringField(payload, "token");
				if (advanceId === void 0 || token === void 0) return internalError("advance-thread-remove endpoint requires advanceId and token payloads");
				try {
					return {
						ok: true,
						value: { threads: await advance.threadRemove(advanceId, token) }
					};
				} catch (error) {
