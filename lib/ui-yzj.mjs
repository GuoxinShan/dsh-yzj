...
const logSchema = z$1.object({
	...
	entries: z$1.array(entrySchema$1)
});
defineDomain({
	name: "yzj_home_logs",
	version: 0,
...
function calendarEventsFromJson(json) {
	if (Array.isArray(json)) return json;
	const record = asRecord$4(json);
	if (Array.isArray(record.list)) return record.list;
	if (Array.isArray(record.data)) return record.data;
	const nested = asRecord$4(record.data);
...
function mergeCalendarEvents(records) {
	...
	for (const record of records) {
		const id = asString$3(asRecord$4(record).id);
	...
	out.sort((left, right) => {
		return (asNumber(asRecord$4(left).startDate) ?? 0) - (asNumber(asRecord$4(right).startDate) ?? 0);
	});
	return out;
}
...
function filterCalendarEvents(records, start, end) {
	...
	return records.filter((record) => {
		const ms = asNumber(asRecord$4(record).startDate);
...
//#endregion
//#region packages/tool-yzj/src/scan-cursors.ts
/**
...
//#endregion
//#region packages/tool-yzj/src/advance-sources.ts
/**
...
//#endregion
//#region packages/tool-yzj/src/advance-dreampool.ts
/**
...
const entryListSchema = z$1.array(entrySchema);
defineDomain({
...
async function openBoundHome(options) {
	...
	const legacyTopicSessionId = options.agents.get(bound.sessionId) === void 0 ? void 0 : await maybeMigrateLegacyHost({
		home: options.home,
		agents: options.agents,
		yzjConversationId: options.yzjConversationId,
		cwd: options.cwd,
...
function rowsOf(json) {
	if (Array.isArray(json)) return json;
	const record = asRecord$3(json);
	if (Array.isArray(record.list)) return record.list;
	if (Array.isArray(record.data)) return record.data;
	if (typeof record.data === "object" && record.data !== null) {
		const inner = asRecord$3(record.data);
		if (Array.isArray(inner.list)) return inner.list;
	}
...
function parseContactUser(json) {
	const user = asRecord$3(rowsOf(json)[0]);
	return {
...
function parseToolArgs$1(raw) {
	if (typeof raw === "string") try {
		return asRecord$2(JSON.parse(raw));
	} catch {
		return {};
	}
	return asRecord$2(raw);
}
...
function writeFileNameOf(data) {
	const rec = asRecord$2(data);
	const name = asString$2(rec.name);
	if (!WRITE_TOOLS$1.has(name)) return void 0;
	const base = fileBaseName(asString$2(parseToolArgs$1(rec.arguments).file_path));
...
async function askTopicAssistant(options) {
	...
	if (window !== "" && !sessionHasSummonWindow(live.session?.events ?? [])) live.inject?.(pluginTurn(window));
...
function assistantTextOf(data) {
	...
	const content = asRecord$1(rec.message).content ?? rec.content;
	if (typeof content === "string") return content.trim();
	if (!Array.isArray(content)) return "";
	const parts = [];
	for (const block of content) {
		...
		if (row.type === "text" && asString$1(row.text).trim() !== "") parts.push(asString$1(row.text).trim());
		else if (asString$1(row.text).trim() !== "") parts.push(asString$1(row.text).trim());
	}
	return parts.join("\n").trim();
}
...
function writePathOf(data) {
	...
	const path = asString$1(parseToolArgs(rec.arguments).file_path);
	return path === "" ? void 0 : path;
}
/** Tool name from one tool/call. */
function toolNameOf(data) {
	return asString$1(asRecord$1(data).name);
}
...
async function deliverTopicResult(options) {
	...
	const content = images.length === 0 ? summary : `${summary}${placeholders === "" ? "" : `\n${placeholders}`}`;
...
var TopicDeliverHub = class {
	...
	noteEvent(sessionId, event) {
		...
		if (event.type === "user/message") {
			if (asRecord$1(asRecord$1(event.data).source).kind !== "plugin") this.stashes.set(sessionId, emptyStash());
			return;
		}
...
function sessionCwd(session) {
	const cwd = asString$1(asRecord$1(asRecord$1(session).meta).cwd);
