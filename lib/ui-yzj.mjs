...
function cliMessageList(json) {
	const payload = unwrapCli$1(json);
...
function mergeCalendarEvents(records) {
	...
	for (const record of records) {
		const id = asString(asRecord$2(record).id);
...
const assistantSchema = z$1.object({
	id: z$1.string().min(1),
...
const bubbleSchema = z$1.object({
	id: z$1.string().min(1),
	role: z$1.enum(["user", "assistant"]),
	text: z$1.string(),
	at: z$1.number()
});
const dmSchema = z$1.object({
	bubbles: z$1.array(bubbleSchema),
	processing: z$1.boolean()
});
const threadSchema = z$1.object({
	groupId: z$1.string().min(1),
	msgId: z$1.string().min(1),
	assistantId: z$1.string().min(1),
	status: z$1.enum(["idle", "processing"]),
	bubbles: z$1.array(bubbleSchema),
	updatedAt: z$1.number()
});
defineDomain({
	...
	version: 0,
	tables: {
		assistants: domainTable(assistantSchema),
		dm: domainTable(dmSchema),
...
//#endregion
//#region packages/tool-yzj/src/home.ts
...
function lastAssistantText(events) {
	for (let index = events.length - 1; index >= 0; index -= 1) {
		...
		const content = (typeof event.data === "object" && event.data !== null ? event.data : {}).content;
...
function cliRows(value) {
	...
	if (Array.isArray(payload)) return payload;
	const rec = asRecord$1(payload);
	if (Array.isArray(rec.list)) return rec.list;
	if (Array.isArray(rec.data)) return rec.data;
	const inner = asRecord$1(rec.data);
	if (Array.isArray(inner.list)) return inner.list;
	return [];
}
/**
...
function identityOf(json) {
	return asRecord(asRecord(json).identity);
}
...
function assistantCwd(assistantId) {
	return join(homedir(), ".dsh-yzj", "assistants", assistantId);
}
/** Create the directory if missing; return the path even when mkdir is denied. */
async function ensureAssistantCwd(assistantId) {
...
function agentsFace$1(ctx) {
	return ctx.get("agents");
}
function liveAgent(ctx, sessionId) {
	const got = agentsFace$1(ctx)?.get(sessionId);
	if (got === void 0) return void 0;
	const inner = got.agent;
	if (inner !== void 0) return {
		...inner.session === void 0 ? {} : { session: inner.session },
		...inner.followup === void 0 ? {} : { followup: inner.followup }
	};
	return {
		...got.session === void 0 ? {} : { session: got.session },
		...got.followup === void 0 ? {} : { followup: got.followup }
	};
}
/**
...
*/
async function ensureAssistantAgent(ctx, sessionId, assistantId) {
	const agents = agentsFace$1(ctx);
	if (agents === void 0) throw new Error("assistant-ask: agents 服务不可用");
	if (agents.get(sessionId) !== void 0) return;
...
function eventsOf(ctx, sessionId) {
...
*/
async function waitTurnEnd(ctx, sessionId, fromLength, timeoutMs) {
	const started = Date.now();
	while (Date.now() - started < timeoutMs) {
		if (eventsOf(ctx, sessionId).slice(fromLength).some((event) => event.type === "turn/end")) return;
		await sleep(250);
	}
}
function groupContext(input) {
	...
	if (input.originWho !== void 0 && input.originWho !== "") lines.push(`Anchor from ${input.originWho}: ${input.originText ?? ""}`);
	if (input.window !== void 0 && input.window !== "") lines.push(input.window);
	return lines.join("\n");
}
/** One IM user turn: enqueue, followup, present-or-fallback, finish. */
async function runAssistantTurn(ctx, assistants, input) {
	const assistant = assistants.store.get(input.target.assistantId) ?? await assistants.store.ensureDefault();
	const sessionId = assistant.sessionId;
	const promptPrefix = assistant.prompt !== void 0 && assistant.prompt !== "" ? `[Assistant notes]\n${assistant.prompt}\n\n` : "";
	const body = input.target.kind === "thread" ? `${promptPrefix}${groupContext({
		groupId: input.target.groupId,
		msgId: input.target.msgId,
		...input.groupName === void 0 ? {} : { groupName: input.groupName },
		...input.originWho === void 0 ? {} : { originWho: input.originWho },
		...input.originText === void 0 ? {} : { originText: input.originText },
		...input.window === void 0 ? {} : { window: input.window }
	})}\n\nUser: ${input.text}` : `${promptPrefix}${input.text}`;
	await assistants.enqueue(assistant.id, async () => {
		...
		await waitTurnEnd(ctx, sessionId, before, 18e4);
		const fallback = lastAssistantText(eventsOf(ctx, sessionId));
	...
	return async (endpoint, payload, _signal) => {
		switch (endpoint) {
			...
			case "assistants-list": {
				const assistants = ctx.get("yzjAssistants");
				if (assistants === void 0) return internalError("assistants-list: yzjAssistants 服务不可用（tool-yzj 未挂载）");
				await assistants.store.ensureDefault();
				return {
					ok: true,
					value: { assistants: assistants.store.list() }
				};
			}
			case "assistants-create": {
				const assistants = ctx.get("yzjAssistants");
				if (assistants === void 0) return internalError("assistants-create: yzjAssistants 服务不可用（tool-yzj 未挂载）");
				const name = stringField(payload, "name") ?? "助手";
				const prompt = stringField(payload, "prompt");
				return {
					ok: true,
					value: { assistant: prompt === void 0 ? await assistants.store.create(name) : await assistants.store.create(name, prompt) }
				};
			}
			case "assistant-ask": {
				const assistants = ctx.get("yzjAssistants");
				if (assistants === void 0) return internalError("assistant-ask: yzjAssistants 服务不可用（tool-yzj 未挂载）");
				const assistantId = stringField(payload, "assistantId") ?? "default";
				const text = stringField(payload, "text");
				if (text === void 0) return internalError("assistant-ask endpoint requires a text payload");
				await assistants.store.ensureDefault();
				if (assistants.store.get(assistantId) === void 0) return internalError(`assistant-ask: unknown assistant ${assistantId}`);
				return {
					ok: true,
					value: await runAssistantTurn(ctx, assistants, {
						target: {
							kind: "dm",
							assistantId
						},
			...
			case "assistant-thread-ask": {
				const assistants = ctx.get("yzjAssistants");
				if (assistants === void 0) return internalError("assistant-thread-ask: yzjAssistants 服务不可用（tool-yzj 未挂载）");
				const assistantId = stringField(payload, "assistantId") ?? "default";
				const groupId = stringField(payload, "groupId");
				const msgId = stringField(payload, "msgId");
				const text = stringField(payload, "text");
				if (groupId === void 0 || msgId === void 0 || text === void 0) return internalError("assistant-thread-ask endpoint requires groupId, msgId, and text");
				await assistants.store.ensureDefault();
				if (assistants.store.get(assistantId) === void 0) return internalError(`assistant-thread-ask: unknown assistant ${assistantId}`);
				const window = homeIoFrom(ctx.get("yzjHome"))?.formatSummonWindow?.(groupId, msgId);
				...
				return {
					...
					value: await runAssistantTurn(ctx, assistants, {
						...
						...window === void 0 || window === "" ? {} : { window }
					})
				};
			}
			case "assistant-projection": {
				const assistants = ctx.get("yzjAssistants");
				if (assistants === void 0) return internalError("assistant-projection: yzjAssistants 服务不可用（tool-yzj 未挂载）");
				await assistants.store.ensureDefault();
				const groupId = stringField(payload, "groupId");
				const msgId = stringField(payload, "msgId");
				if (groupId !== void 0 && msgId !== void 0) return {
					ok: true,
					value: { thread: assistants.store.threadOf(groupId, msgId) }
				};
				const assistantId = stringField(payload, "assistantId") ?? "default";
				const dm = assistants.store.dmProjection(assistantId);
				if (dm === void 0) return internalError(`assistant-projection: unknown assistant ${assistantId}`);
				const writes = writeGate.list(dm.assistant.sessionId).map(projectRecord);
				const threads = typeof payload === "object" && payload !== null && stringField(payload, "threadsGroupId") !== void 0 ? assistants.store.threadsForGroup(stringField(payload, "threadsGroupId") ?? "") : [];
				return {
					ok: true,
					value: {
			...
			case "assistant-threads": {
				const assistants = ctx.get("yzjAssistants");
				if (assistants === void 0) return internalError("assistant-threads: yzjAssistants 服务不可用（tool-yzj 未挂载）");
				const groupId = stringField(payload, "groupId");
				if (groupId === void 0) return internalError("assistant-threads endpoint requires a groupId payload");
				return {
					ok: true,
					value: { threads: assistants.store.threadsForGroup(groupId) }
				};
			}
			case "assistant-process": {
				const assistants = ctx.get("yzjAssistants");
				if (assistants === void 0) return internalError("assistant-process: yzjAssistants 服务不可用（tool-yzj 未挂载）");
				const assistantId = stringField(payload, "assistantId") ?? "default";
...
*/
function apply(ctx) {
	mountYzjRpcChannel(ctx, createRpcHandler(ctx, applyWriteGate(ctx)));
...
*/
function mountYzjRpcChannel(ctx, handler) {
	const webServer = ctx.get("webServer");
	if (webServer === void 0) throw new Error("ui-yzj: webServer service missing (inject declares it)");
	...
	ctx.effect(() => webServer.register({
		...
		handler: async (req, res) => {
			...
			try {
				writeYzjRpc(res, rpcId, await handler(endpoint, body.payload, void 0));
			} catch (error) {
				res.writeHead(500);
...
function yzjEndpointFromPath(pathname) {
	if (!pathname.startsWith(`${YZJ_CHANNEL}/`)) return void 0;
...
function readHttpBody(req, maxBytes = 33554432) {
...
function writeYzjRpc(res, rpcId, result) {
