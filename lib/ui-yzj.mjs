...
*/
function publishHostSession(agent, title, replace = false) {
...
async function openTopicHome(options) {
	...
	const publish = (agent) => {
		publishHostSession(agent, title, true);
	};
	...
	try {
		publish(await options.agents.resume({ resumeSessionId: topic.sessionId }) ?? options.agents.get(topic.sessionId));
		return {
			sessionId: topic.sessionId,
			created: topic.created,
			yzjKind,
...
function topicLensBubbles(topic, agents) {
	const fromHost = topic.fromSessionId === void 0 || topic.fromSessionId === "" ? [] : digestCandidates(sessionEventsOf(agents.get(topic.fromSessionId))).map((row, index) => ({
		...row,
		id: `h${index}`
	}));
		...
		switch (endpoint) {
			...
			case "home-open": {
				...
				try {
					...
					attachYzjSession(ctx, value.sessionId);
					if (value.legacyTopicSessionId !== void 0) attachYzjSession(ctx, value.legacyTopicSessionId);
			...
			case "home-nav": {
				...
				return {
					ok: true,
					value: groupSpaceSnapshot(io, agentsFace(ctx))
				};
			}
			...
			case "home-topic-ask": {
				...
				attachYzjSession(ctx, sessionId);
				return {
					ok: true,
					value: result
				};
			...
			case "home-handoff": {
				...
				if ("error" in result) return internalError(result.error);
				if ("sessionId" in result) attachYzjSession(ctx, result.sessionId);
				const topicId = "topicSessionId" in result ? result.topicSessionId : void 0;
				if (typeof topicId === "string" && topicId !== "") attachYzjSession(ctx, topicId);
