...
function parseThreadToken(token) {
...
function sourceTypeOfThread(prefix) {
...
const threadListSchema = z$1.array(threadSchema);
/** Durable domain: advanceId → subscribed thread rows. */
const yzjAdvanceThreadsDomainSpec = defineDomain({
	name: "yzj_advance_threads",
	version: 0,
	tables: { threads: domainTable(threadListSchema) }
});
/**
* Read/write face over the opened domain. Until `open()`, methods use the
...
var AdvanceThreadStore = class {
	...
	*/
	...
	async add(advanceId, thread) {
		const current = this.threadsOf(advanceId);
		if (current.some((row) => row.token === thread.token)) return {
			added: false,
			threads: current
		};
...
function refsOverlap(incoming, existing) {
...
function buildScanDigest(result) {
	...
	const signalLines = result.signals.length === 0 ? ["新信号：（无）"] : ["新信号：", ...result.signals.map(imMessageLine)];
	...
	const next = result.signals.length === 0 ? "下一步：静默结束本轮。" : "下一步：把新信号交给 yzj_advance_inspect（signals=上列），再按纪律决定是否 feed。";
	return [
		"巡检扫描",
		...groupLines,
		...signalLines,
...
*/
async function coreScanAdvance(ctx, budget, config, caches, cursors, groups, limit = 20, holder, threads) {
	let effective = groups;
	let preItems;
	if (effective.length === 0) {
		if (threads === void 0) throw new Error("advance scan: groups must not be empty (no thread registry)");
		preItems = (await fetchItems(ctx, budget, await resolveAdvance(ctx, budget, config, caches, false, holder))).filter((item) => item.stage !== "completed").map((item) => ({
		...
		for (const [advanceId, rows] of threads.entries()) {
			...
			for (const row of rows) {
				...
				if (parsed !== void 0 && parsed.prefix === "im") channelIds.add(parsed.id);
			}
		}
		if (channelIds.size === 0) throw new Error("advance scan: 没有 open 事项订阅 im: 渠道；先在面板「关联渠道」或 create threads 挂群");
	...
	for (const token of effective) {
		...
		try {
			...
			const fresh = (await listImMessages(ctx, budget, resolved.groupId, "new", prior.lastMsgId, pageSize)).map(parseImMessage).filter((row) => row.msgId !== "" && row.msgId !== prior.lastMsgId);
			...
			for (const row of accepted) signals.push({
				groupId: resolved.groupId,
				groupName: resolved.groupName,
				msgId: row.msgId,
	...
	if (preItems !== void 0) openItems = preItems;
		...
		openItems = [];
		try {
			openItems = (await fetchItems(ctx, budget, await resolveAdvance(ctx, budget, config, caches, false, holder))).filter((item) => item.stage !== "completed").map((item) => ({
				advanceId: item.advanceId,
				title: item.title,
				stage: item.stage
			}));
	...
	const subscriptions = threads === void 0 ? [] : openItems.map((item) => ({
		advanceId: item.advanceId,
		title: item.title,
		stage: item.stage,
...
async function coreCreateAdvance(ctx, budget, config, caches, input, holder, threads) {
	const title = input.title.trim();
	...
	if (threads !== void 0 && input.threads !== void 0 && input.threads.length > 0) {
...
async function coreFeedAdvance(ctx, budget, config, caches, input, holder) {
	...
	if (incomingRefs.length > 0) {
		const hit = (await fetchEntries(ctx, budget, binding, input.advanceId)).find((entry) => refsOverlap(incomingRefs, entry.refs));
...
var YzjAdvanceService = class extends Service {
	...
	cursors;
	threads;
	constructor(ctx, budget, config, holder, cursors = new ScanCursorStore(), threads = new AdvanceThreadStore()) {
	...
	*/
	async threadAdd(advanceId, token, label) {
		const parsed = parseThreadToken(token);
		if (parsed === void 0) throw new Error(`advance: 非法线程 token「${token}」；语法 im:<groupId> / doc:<docId> / todo:<todoId> / event:<eventId> / file:<fileId>`);
		const binding = await resolveAdvance(this.ctx, this.budget, this.config, this.caches, false, this.holder);
		...
		if (!outcome.added) return {
			threads: outcome.threads.map((row) => ({ ...row })),
			entryAppended: false
		};
		if (kind === "document") {
	...
	async threadRemove(advanceId, token) {
		if (parseThreadToken(token) === void 0) throw new Error(`advance: 非法线程 token「${token}」`);
		return (await this.threads.remove(advanceId, token)).map((row) => ({ ...row }));
	}
	...
	async openNow() {
		...
		if (this.threads instanceof AdvanceThreadStore) try {
			await this.threads.open(facility);
		} catch (error) {
...
function applyAdvanceTools(ctx, budget, config, holder, cursors = new ScanCursorStore(), threads = new AdvanceThreadStore()) {
	const caches = {
		lib: {},
		adv: {}
	};
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			stage: {
				...
				description: "open = not completed (default); or one exact stage."
			},
			tag: {
				type: "string",
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			advanceId: {
				type: "string",
				description: "Inspect one item; omit to spread every open (not completed) item."
			},
			signals: {
				type: "string",
		...
		async execute(args) {
			...
			const scoped = wanted === "" ? items.filter((item) => item.stage !== "completed") : items.filter((item) => item.advanceId === wanted);
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			groups: {
				type: "array",
				items: { type: "string" },
				description: "Group ids or names to watch (1–8). Omit to aggregate the im: threads of every open item from the subscription registry."
			},
			limit: {
				type: "number",
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			refs: {
				type: "array",
				items: { type: "string" },
				description: "Traceable ref tokens (yzj:... / msgId / docId) this item originates from; stored on the 立项 entry. Never sent to the CLI."
			},
			sourceType: {
				type: "string",
				enum: [...SOURCE_TYPES],
			...
			threads: {
				type: "array",
				items: { type: "string" },
				description: "Intent-thread tokens to subscribe (im:<groupId> / doc:<docId> / todo:<todoId> / event:<eventId> / file:<fileId>). The founding group usually goes here as 线程①; im: threads drive later yzj_advance_scan aggregation."
			}
		},
		output: yzjToolOutput,
		...
		async execute(args) {
			...
			return {
				content: [
					...
					...threads.threadsOf(result.item.advanceId).length > 0 ? [`已订阅线程：${threads.threadsOf(result.item.advanceId).map((row) => row.token).join("、")}`] : [],
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			detail: {
				type: "string",
				description: "Free-form detail appended after the host-generated field diffs."
			},
			refs: {
				type: "array",
				items: { type: "string" },
				description: "Traceable ref tokens for this signal (yzj:... / msgId / docId / todoId). Never sent to the CLI."
			},
			stageTo: {
				type: "string",
				enum: [...ADVANCE_STAGES],
...
//#endregion
export { AdvanceThreadStore, BoundLogStore, Config, DEFAULT_BOUND_LOG_LIMITS, HomeBindingStore, TopicAnchorStore, ackLocalEntry, apply, applyAppend, applySummonOncePreStep, cliMessageList, cliMessageToEntry, clipLogParam, conversationKindOf, extractSendMsgId, failLocalEntry, formatSummonWindow, homeSessionId, inject, isPluginFollowup, latestUserSourceKind, localMsgId, mergeFused, name, parseThreadToken, robotOutboundEntry, sessionHasSummonWindow, sessionIdFromAssemble, shouldAttachSummonWindow, sourceTypeOfThread, summonWindowText, threadEntries, threadKindOf, topicAnchorKey, topicSessionId, yzjAdvanceThreadsDomainSpec, yzjHomeDomainSpec, yzjHomeLogDomainSpec, yzjTopicDomainSpec };
