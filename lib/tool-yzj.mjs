...
function textValue(content, truncated) {
	return {
		content,
		truncated,
		data: {}
...
const yzjAssistantsDomainSpec = defineDomain({
	name: "yzj_assistants",
	version: 0,
	tables: {
...
var AssistantStore = class {
	...
	dm;
	threads;
	memoryAssistants = /* @__PURE__ */ new Map();
	memoryDm = /* @__PURE__ */ new Map();
	memoryThreads = /* @__PURE__ */ new Map();
	/** Live turn routing — not durable; a restart drops in-flight targets. */
	turnBySession = /* @__PURE__ */ new Map();
	presentedThisTurn = /* @__PURE__ */ new Set();
	async open(facility) {
		if (this.assistants !== void 0) return;
		...
		for (const [key, value] of this.memoryAssistants) if (this.assistants.get(key) === void 0) await this.assistants.put(key, value);
		for (const [key, value] of this.memoryDm) if (this.dm.get(key) === void 0) await this.dm.put(key, value);
		for (const [key, value] of this.memoryThreads) if (this.threads.get(key) === void 0) await this.threads.put(key, value);
	...
	list() {
		if (this.assistants !== void 0) return [...this.assistants.entries()].map(([, record]) => record).sort((a, b) => a.createdAt - b.createdAt);
	...
	async create(name, prompt) {
		...
		const record = {
			...
			createdAt: Date.now(),
			...prompt !== void 0 && prompt.trim() !== "" ? { prompt: prompt.trim().slice(0, 4e3) } : {}
		};
		await this.putAssistant(record);
		return record;
	}
	async setTurn(sessionId, target) {
	...
	*/
	async fallbackPresent(sessionId, text) {
		if (this.presentedThisTurn.has(sessionId)) return void 0;
	...
	dmProjection(assistantId) {
		const assistant = this.get(assistantId);
		if (assistant === void 0) return void 0;
	...
	threadOf(groupId, msgId, assistantId = DEFAULT_ASSISTANT_ID) {
	...
	threadsForGroup(groupId) {
		return (this.threads !== void 0 ? [...this.threads.entries()].map(([, row]) => row) : [...this.memoryThreads.values()]).filter((row) => row.groupId === groupId);
	}
	targetFromSession(sessionId) {
	...
	dmOf(assistantId) {
		return this.dm?.get(assistantId) ?? this.memoryDm.get(assistantId) ?? {
			bubbles: [],
			processing: false
		};
	}
	async setDmProcessing(assistantId, processing) {
		const dm = this.dmOf(assistantId);
		await this.putDm(assistantId, {
			bubbles: dm.bubbles,
			processing
		});
	}
	async setThreadStatus(groupId, msgId, assistantId, status) {
...
var YzjAssistantsService = class extends Service {
	...
	*/
	enqueue(assistantId, job) {
		const next = (this.queues.get(assistantId) ?? Promise.resolve()).then(job, job);
		this.queues.set(assistantId, next.then(() => void 0, () => void 0));
		return next;
	}
};
/**
...
function callingSessionId$1(exec) {
	const id = exec?.agent?.session?.id;
	return typeof id === "string" ? id : "";
}
/** Register `present`. Not a write tool — no confirm card. */
function applyPresentTool(ctx, budget) {
	ctx.tools.register(defineTool({
		name: "present",
		...
		async execute(args, exec) {
			...
			const sessionId = callingSessionId$1(exec);
			const assistants = ctx.get("yzjAssistants");
			if (assistants === void 0 || sessionId === "") return {
				...textValue("present: no assistant session on this turn.", false),
				data: { shown: false }
			};
			const bubble = await assistants.store.present(sessionId, text);
			const shown = bubble !== void 0;
			return {
				...textValue(shown ? `present: shown in IM (${bubble.role} ${bubble.text.length} chars). The user cannot see other tools.` : "present: this session has no IM turn target; the user did not see this text.", false),
				data: {
					shown,
					...bubble === void 0 ? {} : {
						bubbleId: bubble.id,
...
*/
function applyPresentInstruction(ctx) {
	ctx.on("agent/pre-step", (async (payload, next) => {
		const decision = await next();
		if (decision.kind !== "enter") return decision;
		const sessionId = payload.agent?.session?.id;
		if (sessionId === void 0 || !sessionId.startsWith("yzj-assistant-")) return decision;
		const events = payload.agent?.session?.events ?? [];
		const incoming = decision.messages ?? payload.messages ?? [];
		if ([...events, ...incoming.map((item) => ({
			type: "user/message",
			data: item
		}))].some((event) => {
			const data = userMessageData(event);
			return data !== void 0 && textOfUserMessage(data).includes(PRESENT_MARK);
		})) return decision;
		return {
			kind: "enter",
			messages: [{
...
//#endregion
export { AssistantStore, BoundLogStore, Config, DEFAULT_ASSISTANT_ID, DEFAULT_BOUND_LOG_LIMITS, HomeBindingStore, TopicAnchorStore, YzjAssistantsService, ackLocalEntry, apply, applyAppend, applyPresentInstruction, applySummonOncePreStep, assistantSessionId, cliList, cliMessageList, cliMessageToEntry, cliObject, clipLogParam, conversationKindOf, extractSendMsgId, failLocalEntry, formatSummonWindow, homeSessionId, inject, isAssistantSessionId, isPluginFollowup, lastAssistantText, latestUserSourceKind, localMsgId, mergeFused, name, robotOutboundEntry, sessionHasSummonWindow, sessionIdFromAssemble, shouldAttachSummonWindow, summonWindowText, threadEntries, threadKey, topicAnchorKey, topicSessionId, unwrapCli, yzjAssistantsDomainSpec, yzjHomeDomainSpec, yzjHomeLogDomainSpec, yzjTopicDomainSpec };
