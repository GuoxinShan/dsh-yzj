...
function tableFieldsJson() {
	return JSON.stringify([
		...
		},
		{
			name: F.status,
			type: "SingleSelect",
			data: { items: TODO_STATUSES.map((value) => ({ value })) }
		},
		{
			name: F.assignee,
			type: "MultiLineText"
...
var YzjTodoService = class extends Service {
	...
	*/
	async edit(todoId, patch) {
		...
		const wrote = await writeRecords(this.ctx, this.budget, "sheet record update", binding, JSON.stringify([{
			id: todo.recordId,
			fieldsValue: fields
		}]));
		if (!wrote.ok) throw new Error(wrote.content);
...
* migration).
*/
async function scanTodoThread(ctx, budget, config, caches, cursors, thread, signals, groupResults, now, pool, holder) {
	const key = `todo:${thread.id}`;
	let todo;
	try {
		todo = await fetchTodoByTodoId(ctx, budget, await resolveLibrary(ctx, budget, config, caches.lib, false, holder), thread.id);
	} catch (error) {
		groupResults.push({
			groupId: key,
...
async function coreFeedAdvance(ctx, budget, config, caches, input, holder, sources) {
	...
	const existing = incomingRefs.length > 0 || normalizedChangeType === "决策请求" ? await fetchEntries(ctx, budget, binding, input.advanceId) : [];
	let overlappedRefs = [];
