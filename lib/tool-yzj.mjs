...
function localAdvanceStore() {
	if (singleton === void 0) singleton = new AdvanceLocalStore(defaultLocalDbPath());
	return singleton;
}
/**
...
async function fetchItems(ctx, budget, binding) {
	if (advanceBackend === "sqlite") return localAdvanceStore().listItems().map((row) => parseAdvanceItem({
		id: row.recordId,
		fields: row.fields
...
async function fetchEntries(ctx, budget, binding, advanceId) {
	if (advanceBackend === "sqlite") return localAdvanceStore().listEntries(advanceId).map((row) => parseAdvanceEntry({
		id: row.recordId,
		fields: row.fields
...
async function todaysEntryIds(ctx, budget, binding) {
	...
	if (advanceBackend === "sqlite") return localAdvanceStore().listAllEntryIds().filter((id) => id.includes(`E-${day}-`));
	const filter = JSON.stringify({
		mode: "AND",
		criteria: [{
			field: ENTRY_F.id,
