...
*/
async function coreScanAdvance(ctx, budget, config, caches, cursors, groups, limit = 20, holder, threads, pool) {
...
var YzjAdvanceService = class extends Service {
	...
	threads;
	pool;
	constructor(ctx, budget, config, holder, cursors = new ScanCursorStore(), threads = new AdvanceThreadStore(), pool) {
	...
	*/
	...
	dreamState() {
		...
		return {
			pending: this.pool.pending().length,
...
function applyAdvanceTools(ctx, budget, config, holder, cursors = new ScanCursorStore(), threads = new AdvanceThreadStore(), pool) {
	const caches = {
		lib: {},
		adv: {}
	};
