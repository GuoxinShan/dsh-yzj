...
async function scanDirThread(ctx, budget, cursors, dir, signals, groupResults, now) {
	...
	try {
		...
		for (const doc of fresh) signals.push({
			kind: "dir",
			groupId: key,
			groupName: name,
			msgId: doc.id,
...
async function coreScanAdvance(ctx, budget, config, caches, cursors, groups, limit = 20, holder, threads) {
	...
	if (effective.length === 0) {
		...
		if (channelIds.size === 0 && dirIds.size === 0) throw new Error("advance scan: 没有 open 事项订阅 im:/dir: 渠道；先在面板「关联渠道」或 create threads 挂群/目录");
	...
	for (const dir of scanDirs) await scanDirThread(ctx, budget, cursors, dir, signals, groupResults, now);
...
var YzjAdvanceService = class extends Service {
	...
	*/
	async threadAdd(advanceId, token, label) {
		const parsed = parseThreadToken(token);
		if (parsed === void 0) throw new Error(`advance: 非法线程 token「${token}」；语法 im:<groupId> / doc:<docId> / dir:<docId> / todo:<todoId> / event:<eventId> / file:<fileId>`);
		const binding = await resolveAdvance(this.ctx, this.budget, this.config, this.caches, false, this.holder);
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			threads: {
				type: "array",
				items: { type: "string" },
				description: "Intent-thread tokens to subscribe (im:<groupId> / dir:<docId 目录或整库 kbId> / doc:<docId> / todo:<todoId> / event:<eventId> / file:<fileId>). The founding group usually goes here as 线程①; im:/dir: threads drive later yzj_advance_scan aggregation."
			}
		},
		output: yzjToolOutput,
