	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			id: {
				type: "string",
				required: true,
				description: "File id (from yzj_file_upload or an IM attachment)."
			},
			output: {
				type: "string",
...
async function coreCreate(ctx, budget, config, cache, input, holder) {
	...
	return {
		todo: cliRecords(wrote.json).map((record) => parseTodoRecord(record)).find((todo) => todo !== null) ?? null,
		idempotent: false,
		assigneeNote,
		binding
	};
...
function applyAppend(existing, incoming, options = {}) {
	...
	if (hit !== void 0) {
		if (hit.origin === "dsh-send" && incoming.origin !== "dsh-send") return {
			entries: [...existing],
			result: {
				accepted: false,
...
function formatSummonWindow(log, options) {
	...
	const window = log.entries.filter((entry) => {
	...
	for (let index = window.length - 1; index >= 0; index -= 1) {
		...
		const reply = entry.replyMsgId === void 0 ? "" : ` 回复 ${shortReply(log, entry.replyMsgId)}`;
...
function shortReply(log, replyMsgId) {
	...
	if (hit === void 0) return replyMsgId.slice(0, 8);
...
var YzjHomeService = class extends Service {
	...
	ackLocal(yzjConversationId, localId, realMsgId) {
	...
	formatSummonWindow(yzjConversationId, excludeMsgId) {
...
function isBoundHomeSession(sessionId) {
	return sessionId === void 0 || sessionId.startsWith("yzj-home-");
}
...
*/
function applySummonWindow(ctx, home) {
	...
	ctx.effect(() => systemPrompt.context({
		...
		text: (assemble) => {
			...
			if (binding === void 0) return "";
			if (latestUserSourceKind(eventsFromAssemble(assemble)) !== "user") return "";
...
function eventsFromAssemble(assemble) {
	const fromAgent = assemble?.agent?.session?.events;
	if (fromAgent !== void 0) return fromAgent;
	if (assemble?.scope !== void 0 && typeof assemble.scope === "object" && assemble.scope !== null) {
		const scoped = assemble.scope;
		if (scoped.session?.events !== void 0) return scoped.session.events;
	}
	return [];
}
//#endregion
export { BoundLogStore, Config, DEFAULT_BOUND_LOG_LIMITS, HomeBindingStore, ackLocalEntry, apply, applyAppend, cliMessageList, cliMessageToEntry, conversationKindOf, extractSendMsgId, failLocalEntry, formatSummonWindow, homeSessionId, inject, isPluginFollowup, latestUserSourceKind, localMsgId, mergeFused, name, sessionIdFromAssemble, yzjHomeDomainSpec, yzjHomeLogDomainSpec };
