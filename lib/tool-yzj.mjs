...
*/
async function coreScanAdvance(ctx, budget, config, caches, cursors, groups, limit = 20, holder) {
	if (groups.length === 0) throw new Error("advance scan: groups must not be empty");
	if (groups.length > 8) throw new Error(`advance scan: at most 8 groups`);
...
async function coreCreateAdvance(ctx, budget, config, caches, input, holder) {
	const title = input.title.trim();
...
var YzjAdvanceService = class extends Service {
	...
	holder;
	cursors;
	constructor(ctx, budget, config, holder, cursors = new ScanCursorStore()) {
...
function applyAdvanceTools(ctx, budget, config, holder, cursors = new ScanCursorStore()) {
	const caches = {
		lib: {},
		adv: {}
	};
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			groups: {
				...
				required: true,
				items: { type: "string" },
				description: "Group ids or names to watch (1–8). Names resolve through im group recent."
			},
			limit: {
				type: "number",
		...
		async execute(args) {
			...
			if (groups.length === 0) return {
				content: "advance scan: groups must not be empty",
				truncated: false,
				data: {
					kind: "advance-scan",
					ready: false
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			...
			return {
				content: [`created 推进事项 ${result.item.advanceId} · ${result.item.title} [draft]${result.assigneeNote}`, `推进看板 ${result.binding.link}`].join("\n"),
				truncated: false,
				data: {
					kind: "advance-create",
					advanceId: result.item.advanceId,
