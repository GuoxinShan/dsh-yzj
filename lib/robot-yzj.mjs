...
function ensureYzjWorkspaceDirSync() {
...
function homeSessionIdOf(yzjConversationId) {
...
var RobotRouter = class {
	...
	constructor(options) {
		...
		this.cwd = options.cwd !== void 0 && options.cwd !== "" ? options.cwd : yzjWorkspacePath();
	...
	homeFace() {
		if (this.home === void 0) return void 0;
		return typeof this.home === "function" ? this.home() : this.home;
	}
	/** Stamp one delivered robot post into the group-room log (R9). */
	noteRobotPost(surface, msgId, content, replyMsgId) {
		const topicSessionId = surface.sessionId.startsWith("yzj-topic-") ? surface.sessionId : void 0;
		this.homeFace()?.appendLog?.(surface.groupId, {
			msgId,
			sentAt: Date.now(),
			...
			isSelf: false,
			status: "acked",
			...replyMsgId === void 0 || replyMsgId === "" ? {} : { replyMsgId },
			...topicSessionId === void 0 ? {} : { topicSessionId }
		});
		if (surface.sessionId !== "") this.homeFace()?.registerTopicOutbound?.(msgId, surface.sessionId);
	}
	/**
	...
	*/
	async resolveSession(message) {
		...
		if (continued !== void 0) return this.rememberResolved(message, kind, continued);
		if (home?.ensureTopic !== void 0) {
		...
		const sessionId = home === void 0 ? homeSessionIdOf(message.groupId) : SessionId((await home.ensureBound(message.groupId, kind)).sessionId);
		return this.rememberResolved(message, kind, sessionId);
	}
	/** Reply-chain or synthetic continuation target, if any. */
	continuedTopicId(message, home) {
		...
		const fromOutbound = this.outboundAnchor.get(reply.replyMsgId) ?? (reply.replyMsgId === "" ? void 0 : home?.getTopicByOutbound?.(reply.replyMsgId)?.dshSessionId);
		if (fromOutbound !== void 0) return SessionId(String(fromOutbound));
		const rootId = reply.replyRootMsgId !== "" ? reply.replyRootMsgId : reply.replyMsgId;
		if (rootId === "") return void 0;
		const fromRoot = this.inboundAnchor.get(rootId) ?? home?.getTopicByAnchor?.(message.groupId, rootId)?.dshSessionId;
		return fromRoot === void 0 ? void 0 : SessionId(String(fromRoot));
	}
	/** Persist cwd + inbound root mapping for one resolved session. */
	rememberResolved(message, kind, sessionId) {
		if (kind === "group") {
			const cwd = String(sessionId).startsWith("yzj-topic-") ? join(this.cwd, "groups", slug(message.groupId), slug(String(sessionId).slice(10))) : this.groupHomeCwd(message.groupId);
	...
	*/
	...
	async handle(message) {
		...
		if (message.synthetic !== true) await this.noteInboundLog(message);
		...
		if (this.lastSurface !== void 0) this.lastSurface = {
			...this.lastSurface,
	...
	*/
	async noteInboundLog(message) {
		const home = this.homeFace();
		if (home?.appendLog === void 0) return;
		...
		let param;
		if (message.msgParam !== void 0) try {
			const parsed = JSON.parse(message.msgParam);
			if (typeof parsed === "object" && parsed !== null) param = parsed;
		} catch {
			param = void 0;
		}
		try {
			await home.appendLog(message.groupId, {
				...
				status: "acked",
				...reply?.replyMsgId === void 0 || reply.replyMsgId === "" ? {} : { replyMsgId: reply.replyMsgId },
				...param === void 0 || Object.keys(param).length === 0 ? {} : { param }
	...
	*/
	...
	async reply(text, anchor) {
		...
		if (result.ok && result.msgId !== void 0 && this.lastSurface !== void 0) {
	...
	*/
	async dispatchTurn(sessionId, conversationKey, message, turnText) {
		...
		const already = (agent.session?.events ?? []).some((event) => {
			...
			return JSON.stringify(event.data ?? "").includes("［本群最近消息");
		});
		if (window !== "" && !already) try {
			agent.inject(createUserMessage({
				content: [{
					type: "text",
					text: window
	...
	async ensureAgent(sessionId, conversationKey) {
		...
		const composition = this.composePreset === void 0 ? {} : await this.composePreset();
		const meta = {
			cwd,
	...
	async runCommand(command, message, anchor) {
		...
		switch (command) {
			case "help":
				...
				return;
			case "configure":
				await this.reply(this.guiUrl === "" ? "机器人设置：在 DSH「设置 → 云之家」调整模型覆盖与通道状态。" : `机器人设置面板：${this.guiUrl}（设置 → 云之家：模型覆盖、通道状态）`, anchor);
				return;
			case "routines": {
...
*/
async function openOrResumeBoundHome(options) {
	let conversationId = options.home.getBySession(options.sourceSessionId)?.yzjConversationId;
	if (conversationId === void 0) {
		for (const surface of options.surfaces) if (surface.lastSessionId === options.sourceSessionId) {
	...
	const sessionId = (await options.home.ensureBound(conversationId, kind)).sessionId;
	if (options.agents.get(sessionId) !== void 0) return {
		ok: true,
		sessionId,
		createdAgent: false
	};
...
var PushHub = class {
	...
	async sendSafely(conversation, text, notify = false) {
		try {
			const replyMsgId = conversation.noReplyAnchor === true ? void 0 : conversation.lastInbound.msgId;
			...
			if (result.ok && result.msgId !== void 0) conversation.onOutbound?.(result.msgId, text, replyMsgId);
		} catch {}
	}
...
function applyRobotControlTools(ctx, robot) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: { sessionId: {
			type: "string",
			required: true,
			description: "Source session id (bound yzj-home-… from robot_status, or a surface lastSessionId)."
		} },
		output: controlOutput,
		timeoutMs: 6e4,
		isConcurrencySafe: () => false,
...
var YzjRobot = class extends Service {
	...
	*/
	async notify(text, robotIndex = 0) {
		...
		if (result.ok && result.msgId !== void 0) {
			const surface = channel.router.surfaceSummary()[0];
			const home = this.ctx.get("yzjHome");
			if (surface !== void 0 && home?.appendLog !== void 0) {
				const kind = surface.groupId.startsWith("BOT-") ? "dm" : "group";
				await home.ensureBound(surface.groupId, kind);
				const topicSessionId = surface.lastSessionId !== void 0 && surface.lastSessionId.startsWith("yzj-topic-") ? surface.lastSessionId : void 0;
				...
				if (surface.lastSessionId !== void 0) await home.registerTopicOutbound?.(result.msgId, surface.lastSessionId);
			}
		}
		return result;
	}
	...
	*/
	async forkSession(sourceSessionId) {
		...
		if (home === void 0) return {
			ok: false,
			error: "yzjHome 未挂载：无法打开绑定会话（禁止 fork 新根）"
		};
		return openOrResumeBoundHome({
			sourceSessionId,
			home,
			surfaces: this.channels.flatMap((channel) => channel.router.surfaceSummary()),
			agents: {
				get: (id) => this.ctx.agents.get(id),
				resume: (options) => this.ctx.agents.resume({ resumeSessionId: options.resumeSessionId }),
				create: (options) => this.ctx.agents.create({
	...
	*/
	...
	makeChannel(robotConfig, channelIndex) {
		...
		const router = new RobotRouter({
			...
			push: this.hub,
			channelIndex,
			cwd: robotConfig.cwd !== void 0 && robotConfig.cwd !== "" ? robotConfig.cwd : ensureYzjWorkspaceDirSync(),
...
function apply(ctx, config) {
	...
	ctx.on("session/event", (session, event) => {
		robot.noteSessionEvent(String(session.id), event);
	});
