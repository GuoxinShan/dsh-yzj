...
- **Workspace panel** — floating-ball entry (hover quick-dock, persistent toggle), six tabs: 知识库 (two-pane drill-down with doc content preview), 日程 (today), 会话 (full IM: chronological bubbles, media/file previews, emoji, reply, date dividers, anchor tags, 全部已读, plus a real composer sending via `im-send` as user-direct writes), 待办 (urgency buckets 逾期/今天/进行中/待办/已完成, #tag aggregation chips, quick-create parsing `#tag` + date fragments, one-click provisioning, checkbox complete/reopen, draggable rows), 机器人 (channel status with resolved cwd, per-conversation model overrides, and the group shared workspace: browse files / panel-direct write with auto-unique names), and 记忆 (memory vault browser over `ctx.yzjMemory`: expandable sections/entities, open observations, injection stats, the dream log tail, and a panel-direct 记一条 observe composer — user-direct writes, no confirmation card; an unmounted memory-yzj renders an install hint). Unread counts come from CLI `unreadCount` plus locally persisted read-state; browser system notifications on unread increase. All panel entries remain draggable into the composer as reference chips.
