...
- **Tool cards** — one keyed view per yzj tool name (49: contact / doc / sheet / calendar / im / file): pending calls render the family title from args; settled calls render the structured `meta` payload with the digest text as fallback, and an error summary on failure. Card 查看 jumps switch the workbench domain.
