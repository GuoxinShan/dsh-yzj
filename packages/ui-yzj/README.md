...
- **Workbench domains** — four tabs on the workbench top bar (R28). 对话 is the session list + group-room timeline; 待办 / 日程 / 知识库 embed the former panel tabs (todo checkboxes are user-direct, no confirm card). Memory UI is deferred. The sidebar foot is a single 「云之家」entry; it does not switch domains.
