      ...
      <YzjLoginBanner
        authStatus={async (): Promise<Rpc> => {
          ...
          return {
            ok: true,
            value: loggedIn
              ? { loggedIn: true, name: opts.name ?? '测试用户', openId: 'oid', reason: '' }
...
describe('YzjLoginBanner', () => {
  it('shows a one-line status when already logged in', async () => {
    const face = mount({ loggedIn: true, name: '测试用户' })
    await flush()
    expect(face.container.textContent).toContain('已登录 · 测试用户')
  ...
  it('opens the CLI login and confirms after the browser finishes', async () => {
      ...
      root.render(
        <YzjLoginBanner
          authStatus={async () => ({
            ok: true,
            value: loggedIn
              ? { loggedIn: true, name: '测试用户', openId: 'oid', reason: '' }
    ...
    expect(container.textContent).toContain('已登录 · 测试用户')
  })
