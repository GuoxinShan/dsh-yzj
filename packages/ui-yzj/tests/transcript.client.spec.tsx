...
describe('YzjFusedView', () => {
  it('renders inbound, dsh-send, and 发给 Claude in one stream', async () => {
    const fused: Rpc = {
      ok: true,
      value: {
        bound: true,
        ...
        items: [
          ...
          { kind: 'session', time: 4, hide: false, event: { type: 'user/message', time: 4, data: { source: { kind: 'user' }, content: [{ type: 'text', text: '对 Claude' }] } } },
          { kind: 'session', time: 5, hide: false, event: { type: 'assistant/message', time: 5, data: { content: [{ type: 'text', text: 'Claude 回复' }] } } },
        ],
      },
    }
    ...
    expect(text).toContain('对 Claude')
    expect(text).toContain('Claude 回复')
