...
/** Max messages pulled per group per scan (翻页取完增量的防爆上限;830 实验发现单页 20 截断丢信号)。 */
...
export function applyAdvanceTools(
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      detail: { type: 'string', description: 'Free-form detail appended after the host-generated field diffs. 决策请求时: 问题分析(多行) + 动作行(每行一个 `动作: 建待办|发消息|定会议 | 键: 值 | …`, 决策 41)。' },
